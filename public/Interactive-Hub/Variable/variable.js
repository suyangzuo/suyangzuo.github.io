...
class 内存Canvas渲染器 {
  ...
  处理鼠标移动(e) {
    ...
    for (const [应用名称, 应用数据] of 内存占用表) {
      if (应用数据.起始位置 >= 0 && 应用数据.起始位置 < 内存容量) {
        ...
        if (x >= 抬头X && x <= 抬头X + 抬头宽度 && y >= 抬头Y && y <= 抬头Y + 抬头高度) {
          悬停应用 = 应用名称;
          break;
        }
      }
  ...
  绘制内存块文本(x, y, 内存数据) {
    if (内存数据.已占用) {
      ...
      // 绘制空闲内存序号
      this.ctx.font = this.字体;
      this.ctx.fillStyle = 内存数据.索引 === this.悬停索引 ? "#fffa" : "#fff3";
      this.ctx.shadowColor = "black";
      this.ctx.shadowBlur = 2;
  ...
  绘制应用抬头(x, y, 内存数据) {
    ...
    const 鼠标在抬头区域 = 鼠标X >= 抬头X && 鼠标X <= 抬头X + 抬头宽度 && 鼠标Y >= 抬头Y && 鼠标Y <= 抬头Y + 抬头高度;
...
function 向内存中添加应用(应用键值对) {
  ...
  if (可用内存块索引列表.length === 0) {
    console.warn(
      `内存分配失败: 应用 ${应用键值对[0]} 需要 ${应用占用内存} 字节，但最大空闲块只有 ${Math.max(
        ...空闲内存表.map((m) => m.容量)
      )} 字节`
    );
    return false;
...
function 检查并分配代码输入分区内存() {
  ...
  if (第二个类型框 && 第二个标识符框 && 第二个值框) {
    ...
    if (当前作用 === "初始化") {
      ...
      第二个分区合法 =
        第二个标识符有字 && 第二个标识符正确 && 第二个值.length > 0 && 标识符相同 && 第一个类型有效 && 第二个值类型有效;
    }
