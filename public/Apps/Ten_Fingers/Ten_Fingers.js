...
let 待向上滚动 = false; // 标记是否需要延迟向上滚动
let 待滚动目标位置 = 0; // 延迟滚动的目标位置

const 字符状态枚举 = Object.freeze({
  默认: "",
  正确: "correct",
  错误: "error",
});
...
function 设置字符结果状态(字符元素, 状态 = 字符状态枚举.默认) {
  if (!字符元素) return;
  const 目标状态 = 状态 || 字符状态枚举.默认;
  if (目标状态) {
    if (字符元素.dataset.state !== 目标状态) {
      字符元素.dataset.state = 目标状态;
    }
  } else if (字符元素.dataset.state) {
    delete 字符元素.dataset.state;
  }
}

function 获取字符结果状态(字符元素) {
  if (!字符元素) return 字符状态枚举.默认;
  return 字符元素.dataset.state || 字符状态枚举.默认;
}

function 设置字符激活状态(字符元素, 是否当前) {
  if (!字符元素) return;
  if (是否当前) {
    if (字符元素.dataset.active !== 当前字符激活标记) {
      字符元素.dataset.active = 当前字符激活标记;
    }
  } else if (字符元素.dataset.active) {
    delete 字符元素.dataset.active;
  }
}
...
function 恢复块状态(块索引) {
  ...
  for (let i = 块数据.startIndex; i < 块数据.endIndex; i++) {
    ...
    // 恢复错误标记
    if (错误字符集合[i]) {
      设置字符结果状态(字符元素, 字符状态枚举.错误);
    } else if (i < 当前输入索引) {
      // 已输入但未标记为错误的字符，标记为正确
      设置字符结果状态(字符元素, 字符状态枚举.正确);
    } else {
      设置字符结果状态(字符元素, 字符状态枚举.默认);
    }
...
function 滚动到当前字符() {
  ...
  requestAnimationFrame(() => {
    ...
    if (字符相对顶部 < 可见区域顶部) {
      ...
      if (滚动差异 > 5) {
        // 设置延迟滚动标志，不立即滚动，等待下次退格时再滚动
        待向上滚动 = true;
        待滚动目标位置 = Math.max(0, 目标滚动位置);
      }
...
function 处理键盘输入(event) {
  ...
  if (event.key === "Backspace") {
    ...
    if (当前输入索引 > 0) {
      ...
      const 当前字符元素 = 获取字符元素(当前输入索引);
      if (当前字符元素) {
        const 当前状态 = 获取字符结果状态(当前字符元素);
        if (当前状态 === 字符状态枚举.正确) {
          正确字符数--;
        } else if (当前状态 === 字符状态枚举.错误) {
          错误字符数--;
        }

        设置字符结果状态(当前字符元素, 字符状态枚举.默认);
        // 清除 data-input 属性
...
function 处理输入字符(输入的字符) {
  ...
  // 如果待向上滚动为true，但用户输入了字符（而不是按退格），则重置标志
  if (待向上滚动) {
    待向上滚动 = false;
    待滚动目标位置 = 0;
  }
  ...
  for (let i = 0; i < 输入的字符.length; i++) {
    ...
    if (实际输入的字符 === 应该输入的字符) {
      设置字符结果状态(当前字符元素, 字符状态枚举.正确);
      正确字符数++;
    } else {
      设置字符结果状态(当前字符元素, 字符状态枚举.错误);
      错误字符数++;
