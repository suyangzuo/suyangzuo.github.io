class 随心绘 {
  constructor() {
    ...
    this.基础形状单选框组 = {
      ...
      正多边形: document.getElementById("正多边形"),
      正多角星: document.getElementById("正多角星"),
  ...
  添加canvas鼠标移动事件() {
    this.canvas.addEventListener("mousemove", (e) => {
      ...
      if (this.全局属性.已选中基础形状 === "矩形" || this.全局属性.已选中基础形状 === "选框") {
        ...
        this.当前形状对象.边数 = this.全局属性.正多边形边数;
        this.当前形状对象.顶点坐标组 = this.获取正多边形顶点坐标组(
          this.全局属性.点击坐标.x,
          this.全局属性.点击坐标.y,
          半径,
          this.当前形状对象.边数,
        ...
        if (this.全局标志.辅助视觉效果) {
          ...
          this.描边辅助正圆(this.全局属性.点击坐标.x, this.全局属性.点击坐标.y, this.当前形状对象.尺寸.半径);
        }
        ...
        this.当前形状对象.尺寸.外半径 = 外半径;
        if (!this.全局标志.手动调整内半径) {
          this.当前形状对象.尺寸.内半径 = 外半径 * 0.5;
        }
        const 边数 = this.全局属性.正多边形边数;
        this.当前形状对象.边数 = this.全局属性.正多边形边数;
        this.当前形状对象.外顶点坐标组 = this.获取正多边形顶点坐标组(
          this.全局属性.点击坐标.x,
          this.全局属性.点击坐标.y,
          this.当前形状对象.尺寸.外半径,
          this.当前形状对象.边数,
        ...
        this.当前形状对象.内顶点坐标组 = this.获取正多边形顶点坐标组(
          this.全局属性.点击坐标.x,
          this.全局属性.点击坐标.y,
          this.当前形状对象.尺寸.内半径,
          this.当前形状对象.边数,
        ...
        if (this.全局标志.辅助视觉效果) {
          ...
          this.描边辅助正圆(this.全局属性.点击坐标.x, this.全局属性.点击坐标.y, this.当前形状对象.尺寸.外半径);
          this.描边辅助正圆(this.全局属性.点击坐标.x, this.全局属性.点击坐标.y, this.当前形状对象.尺寸.内半径);
        }
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (this.全局标志.左键已按下 && this.全局属性.已选中基础形状 === "矩形") {
        ...
        if (e.key === "[") {
          ...
          if (this.当前形状对象.尺寸.内半径 >= 2) {
            this.当前形状对象.尺寸.内半径 -= 2;
          }
        }
        if (e.key === "]") {
          this.全局标志.手动调整内半径 = true;
  ...
  更新正圆路径(正圆对象) {
    正圆对象.路径 = new Path2D();
    path.arc(正圆对象.坐标.x, 正圆对象.坐标.y, 正圆对象.尺寸.半径, 0, 2 * Math.PI);
  }
  ...
  更新正多边形路径(正多边形对象) {
    if (正多边形对象.顶点坐标组.length < 3) return;
    正多边形对象.路径 = new Path2D();
    正多边形对象.顶点坐标组 = this.获取正多边形顶点坐标组(
      正多边形对象.坐标.x,
      正多边形对象.坐标.y,
      正多边形对象.尺寸.半径,
      正多边形对象.边数,
    ...
    正多边形对象.路径.moveTo(正多边形对象.顶点坐标组[0].x, 正多边形对象.顶点坐标组[0].y);
    for (let i = 1; i < 正多边形对象.顶点坐标组.length; i++) {
      正多边形对象.路径.lineTo(正多边形对象.顶点坐标组[i].x, 正多边形对象.顶点坐标组[i].y);
    }
    正多边形对象.路径.closePath();
  }

  更新正多角星路径(正多角星对象) {
    if (正多角星对象.外顶点坐标组.length < 3 || 正多角星对象.内顶点坐标组.length < 3) return;
    正多角星对象.路径 = new Path2D();
    const 边数 = 正多角星对象.外顶点坐标组.length;
    正多角星对象.外顶点坐标组 = this.获取正多边形顶点坐标组(
      正多角星对象.坐标.x,
      正多角星对象.坐标.y,
      正多角星对象.尺寸.外半径,
      边数,
    ...
    正多角星对象.内顶点坐标组 = this.获取正多边形顶点坐标组(
      正多角星对象.坐标.x,
      正多角星对象.坐标.y,
      正多角星对象.尺寸.内半径,
      边数,
      正多角星对象.起始弧度 + Math.PI / 边数
    );
    for (let i = 0; i < 边数; i++) {
      if (i === 0) {
        正多角星对象.路径.moveTo(正多角星对象.外顶点坐标组[i].x, 正多角星对象.外顶点坐标组[i].y);
      } else {
        正多角星对象.路径.lineTo(正多角星对象.外顶点坐标组[i].x, 正多角星对象.外顶点坐标组[i].y);
      }
      正多角星对象.路径.lineTo(正多角星对象.内顶点坐标组[i].x, 正多角星对象.内顶点坐标组[i].y);
    }
    正多角星对象.路径.closePath();
  }
  ...
  更新路径(路径对象) {
    if (路径对象.形状 === "矩形") {
      ...
      this.更新椭圆路径(路径对象);
    } else if (路径对象.形状 === "正多边形") {
      this.更新正多边形路径(路径对象);
    } else if (路径对象.形状 === "正多角星") {
      this.更新正多角星路径(路径对象);
    } else if (路径对象.形状 === "直线") {
      this.更新直线路径(路径对象);
    } else if (路径对象.形状 === "自由") {
