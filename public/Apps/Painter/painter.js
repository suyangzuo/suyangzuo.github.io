class 随心绘 {
  constructor() {
    ...
    this.全局标志 = {
      ...
      ESC确认文本: false, // ESC确认文本，防止立即触发取消选择
    };
    ...
    this.全局属性 = {
      ...
      拖拽编组ID: null, // 正在拖拽的编组ID
      初始交互框尺寸: null, // 拖拽开始时的交互框尺寸 { width, height }
      初始交互框位置: null, // 拖拽开始时的交互框位置（中心点） { x, y }
      框选预览编组ID集合: new Set(), // 框选预览中的编组ID集合
      ...
      ESC按下时间: null, // ESC键按下的时间戳（用于文本编辑模式，仅文本工具添加）
      文本背景矩形: null, // 文本背景矩形对象（ESC按下超过500ms时创建，仅文本工具添加）
    };
  ...
  // 内部函数：只删除组数据和组属性，不清除形状的所属组ID
  _删除组数据(组ID) {
    if (!组ID || !this.数据集.形状组[组ID]) return;

    // 删除组属性
    if (this.数据集.组属性[组ID]) {
      delete this.数据集.组属性[组ID];
    }
  ...
  // 检测鼠标是否在编组内（通过组ID或边界对象）
  鼠标位于编组内(组ID或边界) {
    if (!this.全局属性.鼠标坐标) return false;

    let 边界;
    if (typeof 组ID或边界 === "object" && 组ID或边界.minX !== undefined) {
      // 如果传入的是边界对象，直接使用
      边界 = 组ID或边界;
    } else {
      // 如果传入的是组ID，需要计算边界
      let 组ID;
      if (typeof 组ID或边界 === "object") {
        // 如果传入的是形状对象
        if (!this.形状已编组(组ID或边界)) return false;
        组ID = 组ID或边界.所属组ID;
      } else {
        组ID = 组ID或边界;
      }

      if (组ID === undefined) return false;

      const 组内形状 = this.获取组内所有形状(组ID);
      if (!组内形状 || 组内形状.length < 2) return false;

      // 临时将组内形状设置为多选形状组，以便计算边界
  ...
  根据选中形状索引修改处理按钮状态() {
    ...
    // 均匀分布按钮：需要>=3个单位才能分布（编组视为一个单位）
    // 将多选形状组转换为单位数组，检查单位数量
    let 单位数量 = 多选数量;
    if (多选数量 >= 2) {
      // 检查是否有编组，如果有，需要计算实际单位数量
      const 单位数组 = this.将多选形状组转换为单位数组();
      单位数量 = 单位数组.length;
    }
  ...
  鼠标位于形状内() {
    ...
    // 检查是否满足条件：多选内包含编组、鼠标位于编组内、Shift已按下
    // 注意：只有当"多选内包含编组且不是整个多选就是一个编组"时，才跳过编组内形状的悬停检测
    // 如果整个多选就是一个编组，应该允许单独移动编组内的形状
    let 多选内包含编组 = false;
    let 鼠标在编组内 = false;
    let 鼠标所在编组ID = null;
    let 所有形状属于同一编组 = false;

    if (this.全局属性.多选形状组.length > 0 && this.全局标志.Shift已按下) {
      // 检查多选内是否包含编组
      多选内包含编组 = this.全局属性.多选形状组.some((形状) => this.形状已编组(形状));
      ...
      if (多选内包含编组) {
        ...
        所有形状属于同一编组 =
          多选数量 >= 2 &&
          this.全局属性.多选形状组.every(
            (形状) => this.形状已编组(形状) && 形状.所属组ID === this.全局属性.多选形状组[0].所属组ID,
          );

        // 只有当"多选内包含编组且不是整个多选就是一个编组"时，才检查鼠标是否在编组内
        if (!所有形状属于同一编组) {
          // 检查鼠标是否在某个编组内
          const 已处理的组ID集合 = new Set();
          for (const 形状 of this.全局属性.多选形状组) {
            if (this.形状已编组(形状)) {
              const 组ID = 形状.所属组ID;
              if (!已处理的组ID集合.has(组ID)) {
                已处理的组ID集合.add(组ID);
                if (this.鼠标位于编组内(组ID)) {
                  鼠标在编组内 = true;
                  鼠标所在编组ID = 组ID;
    ...
    for (let i = this.数据集.基础形状对象组.length - 1; i >= 0; i--) {
      ...
      // 如果满足条件（多选内包含编组、鼠标位于编组内、Shift已按下、且不是整个多选就是一个编组），且当前形状属于该编组，则跳过
      if (
        多选内包含编组 &&
        !所有形状属于同一编组 &&
        鼠标在编组内 &&
    ...
    this.canvas.addEventListener("mousemove", (e) => {
      ...
      if (this.全局属性.已选中基础形状 === "选择路径") {
        ...
        if (this.全局标志.正在框选) {
          ...
          // 处理编组：如果框选包含编组内任意一个形状，就显示整个编组的预览
          const 已处理的组ID集合 = new Set();

          for (const 形状 of 框选内的形状) {
            if (this.形状已编组(形状)) {
              const 组ID = 形状.所属组ID;
              if (!已处理的组ID集合.has(组ID)) {
                已处理的组ID集合.add(组ID);
                ...
                if (组内形状 && 组内形状.length >= 2) {
                  // 只要框选包含编组内任意一个形状，就标记该编组为预览状态
                  this.全局属性.框选预览编组ID集合.add(组ID);
                  // 标记组内所有形状为预览状态
                  组内形状.forEach((组内形状对象) => {
                    组内形状对象.框选预览中 = true;
                  });
        ...
        if (
          ...
          this.全局属性.多选旋转初始位置组.forEach((初始位置) => {
            ...
            if (形状.形状 === "圆" || 形状.形状 === "多边形" || 形状.形状 === "多角星") {
              ...
              // 文本：从按下时顶点坐标组计算极值坐标的中心（与矩形、图像、箭头一致）
            ...
            if (形状.形状 === "矩形" || 形状.形状 === "图像" || 形状.形状 === "箭头") {
              ...
              // 通过旋转顶点坐标组，然后从顶点坐标组反推坐标（与单个文本旋转一致）
              if (初始位置.按下时顶点坐标组 && 形状.顶点坐标组) {
                for (let i = 0; i < 初始位置.按下时顶点坐标组.length; i++) {
                  形状.顶点坐标组[i] = this.旋转坐标(初始位置.按下时顶点坐标组[i], 锚点, -弧度差);
                }
              }

              // 从顶点坐标组反推坐标（左边中心点）
              const 中心x =
                (形状.顶点坐标组[0].x + 形状.顶点坐标组[1].x + 形状.顶点坐标组[2].x + 形状.顶点坐标组[3].x) / 4;
              const 中心y =
                (形状.顶点坐标组[0].y + 形状.顶点坐标组[1].y + 形状.顶点坐标组[2].y + 形状.顶点坐标组[3].y) / 4;
              ...
              if (形状.旋转弧度) {
                ...
                const 局部x坐标 = 局部顶点.map((v) => v.局部x);
                实际宽度 = Math.max(...局部x坐标) - Math.min(...局部x坐标);
              } else {
                实际宽度 = Math.max(...形状.顶点坐标组.map((v) => v.x)) - Math.min(...形状.顶点坐标组.map((v) => v.x));
              }
        ...
        if (
          ...
          if (this.全局标志.Shift单独拖拽中 && this.全局属性.Shift点击待移除形状) {
            ...
            if (初始位置) {
              ...
              // 如果按住Ctrl键，限制为水平或垂直移动
              偏移量 = this.限制移动方向(偏移量, this.键盘状态.Control);

              // 只移动这个形状
              // 对于文本，如果旋转过，应该通过顶点坐标组移动，然后从顶点坐标组反推坐标（与多选旋转一致）
              if (单独移动形状.形状 === "文本") {
                if (初始位置.按下时顶点坐标组 && 初始位置.按下时顶点坐标组.length > 0) {
                  // 通过顶点坐标组移动（支持旋转后的文本）
                  ...
                  let 实际宽度;
                  if (单独移动形状.旋转弧度) {
                    const 旋转弧度 = 单独移动形状.旋转弧度;
                    const cos = Math.cos(-旋转弧度);
                    const sin = Math.sin(-旋转弧度);
                    const 局部顶点 = 单独移动形状.顶点坐标组.map((v) => {
                      const dx = v.x - 中心x;
                      const dy = v.y - 中心y;
                      return {
                        局部x: dx * cos - dy * sin,
                    ...
                    实际宽度 =
                      Math.max(...单独移动形状.顶点坐标组.map((v) => v.x)) -
                      Math.min(...单独移动形状.顶点坐标组.map((v) => v.x));
                  }
                  ...
                  if (单独移动形状.旋转弧度) {
                    ...
                    const cos = Math.cos(旋转弧度);
                    const sin = Math.sin(旋转弧度);
                    单独移动形状.坐标.x = 中心x - 半宽 * cos;
                    单独移动形状.坐标.y = 中心y - 半宽 * sin;
                  } else {
                    单独移动形状.坐标.x = 中心x - 半宽;
                    单独移动形状.坐标.y = 中心y;
                  }
                ...
                if (初始位置) {
                  ...
                  if (待移除形状.形状 === "文本") {
                    if (初始位置.按下时顶点坐标组 && 初始位置.按下时顶点坐标组.length > 0) {
                      ...
                      let 实际宽度;
                      if (待移除形状.旋转弧度) {
                        const 旋转弧度 = 待移除形状.旋转弧度;
                        const cos = Math.cos(-旋转弧度);
                        const sin = Math.sin(-旋转弧度);
                        const 局部顶点 = 待移除形状.顶点坐标组.map((v) => {
                          const dx = v.x - 中心x;
                          const dy = v.y - 中心y;
                          return {
                            局部x: dx * cos - dy * sin,
                        ...
                        实际宽度 =
                          Math.max(...待移除形状.顶点坐标组.map((v) => v.x)) -
                          Math.min(...待移除形状.顶点坐标组.map((v) => v.x));
                      }
                      ...
                      if (待移除形状.旋转弧度) {
                        ...
                        const cos = Math.cos(旋转弧度);
                        const sin = Math.sin(旋转弧度);
                        待移除形状.坐标.x = 中心x - 半宽 * cos;
                        待移除形状.坐标.y = 中心y - 半宽 * sin;
                      } else {
                        待移除形状.坐标.x = 中心x - 半宽;
                        待移除形状.坐标.y = 中心y;
                      }
          ...
          // 移动所有多选形状（如果设置了拖拽编组ID，只移动属于该编组的形状，实现编组整体移动）
          this.全局属性.多选初始位置组.forEach((初始位置) => {
            const 形状 = 初始位置.形状;

            // 如果设置了拖拽编组ID，只移动属于该编组的形状（编组整体移动）
            if (this.全局属性.拖拽编组ID !== null) {
              if (!this.形状已编组(形状) || 形状.所属组ID !== this.全局属性.拖拽编组ID) {
                return; // 跳过不属于该编组的形状
              }
            }

            // 对于文本，通过顶点坐标组移动（与单独文本形状移动一致）
            if (形状.形状 === "文本" && 初始位置.按下时顶点坐标组) {
              // 文本：通过顶点坐标组移动（与其他形状一致）
        ...
        if (this.交互框) {
          ...
          if (this.全局标志.拖拽中) {
            ...
            if (
              ...
              // 如果按住Ctrl键，限制为水平或垂直移动
              鼠标偏移量 = this.限制移动方向(鼠标偏移量, this.键盘状态.Control);

              // 使用限制后的鼠标偏移量更新坐标
              // 坐标 = 初始坐标 + (限制后的鼠标偏移量)
              // 其中：初始坐标 = 点击坐标 + 偏移量
              this.全局属性.选中形状.坐标.x = this.全局属性.点击坐标.x + this.全局属性.偏移量.x + 鼠标偏移量.x;
              this.全局属性.选中形状.坐标.y = this.全局属性.点击坐标.y + this.全局属性.偏移量.y + 鼠标偏移量.y;
            } else {
              let 偏移量 = {
                x: this.全局属性.鼠标坐标.x - this.全局属性.点击坐标.x,
                y: this.全局属性.鼠标坐标.y - this.全局属性.点击坐标.y,
              ...
              if (this.全局属性.选中形状.形状 === "文本" && this.全局属性.选中形状.按下时顶点坐标组) {
                // 文本：通过顶点坐标组移动（与其他形状一致）
                for (let i = 0; i < this.全局属性.选中形状.顶点坐标组.length; i++) {
                  this.全局属性.选中形状.顶点坐标组[i].x = this.全局属性.选中形状.按下时顶点坐标组[i].x + 偏移量.x;
                  this.全局属性.选中形状.顶点坐标组[i].y = this.全局属性.选中形状.按下时顶点坐标组[i].y + 偏移量.y;
                }
                // 从顶点坐标组反推坐标（左边中心点）
                const 中心x =
                  (this.全局属性.选中形状.顶点坐标组[0].x +
                    this.全局属性.选中形状.顶点坐标组[1].x +
                    this.全局属性.选中形状.顶点坐标组[2].x +
                    this.全局属性.选中形状.顶点坐标组[3].x) /
                  4;
                const 中心y =
                  (this.全局属性.选中形状.顶点坐标组[0].y +
                    this.全局属性.选中形状.顶点坐标组[1].y +
                    this.全局属性.选中形状.顶点坐标组[2].y +
                    this.全局属性.选中形状.顶点坐标组[3].y) /
                  4;

                // 从顶点坐标组计算实际宽度
                let 实际宽度;
                if (this.全局属性.选中形状.旋转弧度) {
                  const 旋转弧度 = this.全局属性.选中形状.旋转弧度;
                  const cos = Math.cos(-旋转弧度);
                  const sin = Math.sin(-旋转弧度);
                  const 局部顶点 = this.全局属性.选中形状.顶点坐标组.map((v) => {
                    const dx = v.x - 中心x;
                    const dy = v.y - 中心y;
                    return {
                      局部x: dx * cos - dy * sin,
                  ...
                  实际宽度 =
                    Math.max(...this.全局属性.选中形状.顶点坐标组.map((v) => v.x)) -
                    Math.min(...this.全局属性.选中形状.顶点坐标组.map((v) => v.x));
                }
                ...
                if (this.全局属性.选中形状.旋转弧度) {
                  ...
                  const cos = Math.cos(旋转弧度);
                  const sin = Math.sin(旋转弧度);
                  this.全局属性.选中形状.坐标.x = 中心x - 半宽 * cos;
                  this.全局属性.选中形状.坐标.y = 中心y - 半宽 * sin;
                } else {
                  this.全局属性.选中形状.坐标.x = 中心x - 半宽;
                  this.全局属性.选中形状.坐标.y = 中心y;
                }

                // 更新尺寸.宽，使其与顶点坐标组的实际宽度一致
                this.全局属性.选中形状.尺寸.宽 = 实际宽度;

                // 立即更新极值坐标和中心，确保绘制时使用正确的中心点
                this.全局属性.选中形状.极值坐标 = this.获取极值坐标(this.全局属性.选中形状);
                this.全局属性.选中形状.中心 = this.全局属性.选中形状.极值坐标.中心;
              } else if (this.全局属性.选中形状.形状 === "文本" && this.全局属性.选中形状.按下时坐标) {
                // 如果没有按下时顶点坐标组（旧代码兼容），使用原来的逻辑
            ...
            if (this.全局标志.缩放中) {
              ...
              if (
                ...
                // 文本：通过旋转顶点坐标组，然后从顶点坐标组反推坐标（与移动和缩放一致）
                ...
                // 通过旋转顶点坐标组，然后从顶点坐标组反推坐标（与移动和缩放一致）
                if (this.全局属性.选中形状.点击时顶点坐标组 && this.全局属性.选中形状.顶点坐标组) {
                  for (let i = 0; i < this.全局属性.选中形状.点击时顶点坐标组.length; i++) {
                    this.全局属性.选中形状.顶点坐标组[i] = this.旋转坐标(
                      this.全局属性.选中形状.点击时顶点坐标组[i],
      ...
      if (this.全局属性.选中形状 && this.交互框) {
        ...
        if (!this.交互框.鼠标位于内部) {
          ...
          if (!this.全局属性.悬停形状) {
            ...
            if (鼠标在内形状) {
              ...
              // 保存初始交互框信息（用于Ctrl+拖拽绘制轨道）- 必须在设置拖拽标志之前保存
              // 确保极值坐标存在
              if (!this.全局属性.选中形状.极值坐标) {
                this.全局属性.选中形状.极值坐标 = this.获取极值坐标(this.全局属性.选中形状);
              }
              // 如果极值坐标仍然不存在，再次尝试获取
              if (!this.全局属性.选中形状.极值坐标) {
              ...
              const 交互框尺寸 = this.计算交互框尺寸(this.全局属性.选中形状);
              if (交互框尺寸) {
                this.全局属性.初始交互框尺寸 = { ...交互框尺寸 };
                const 极值 = this.获取极值坐标(this.全局属性.选中形状);
                if (极值 && 极值.中心) {
                  this.全局属性.初始交互框位置 = { x: 极值.中心.x, y: 极值.中心.y };
                }
              }
        ...
        if (this.全局标志.Shift已按下 && 鼠标在内形状) {
          ...
          if (鼠标在内形状.已多选) {
            ...
            // 保存初始交互框信息
            const 交互框尺寸 = this.计算多选交互框尺寸();
            if (交互框尺寸) {
              this.全局属性.初始交互框尺寸 = { ...交互框尺寸 };
              const 边界 = this.获取多选形状的边界();
              if (边界) {
                this.全局属性.初始交互框位置 = { x: 边界.centerX, y: 边界.centerY };
              }
            }
          ...
          if (this.全局属性.多选形状组.length > 0) {
            ...
            if (多选边界位置) {
              ...
              if (this.键盘状态.Control && 多选边界位置.位于句柄) {
                ...
                // 获取当前边界（用于计算锚点）
                // 注意：锚点应该基于当前边界，而不是初始边界
                // 初始边界只用于计算旋转增量，如果旋转后移动了，锚点应该使用移动后的位置
                const 当前边界 = this.获取多选形状的边界();

                if (当前边界) {
                  // 锚点使用当前边界（旋转后移动后的实际位置）
                  const 锚点 = {
                    x: 当前边界.minX + 当前边界.width / 2,
                    y: 当前边界.minY + 当前边界.height / 2,
                  };
                  this.全局属性.多选旋转锚点 = 锚点;

                  // 记录初始边界中心（用于计算整体旋转角度）
                  // 如果有保存的初始边界，使用保存的；否则使用当前边界
                  const 用于计算旋转的边界 = 初始边界 || 当前边界;
                  this.全局属性.多选旋转初始边界中心 = {
                    x: 用于计算旋转的边界.minX + 用于计算旋转的边界.width / 2,
                    y: 用于计算旋转的边界.minY + 用于计算旋转的边界.height / 2,
                  };
                  // 记录初始边界（未旋转状态，用于计算旋转增量）
                  if (初始边界) {
                    this.全局属性.多选旋转初始边界 = structuredClone(初始边界);
                  } else {
                    // 如果没有保存的初始边界，使用当前边界作为初始边界
                    this.全局属性.多选旋转初始边界 = structuredClone(当前边界);
                  }
                  // 初始化整体旋转弧度为0
                  this.全局属性.多选整体旋转弧度 = 0;
                ...
                if (边界) {
                  ...
                  this.全局属性.多选缩放初始位置组 = this.全局属性.多选形状组.map((形状) => {
                    const 初始位置 = {
                      ...
                      按下时缩放比例: 形状.缩放比例 ? { ...形状.缩放比例 } : { x: 1, y: 1 },
                    };
                    // 为文本形状记录初始字号和尺寸
                    if (形状.形状 === "文本") {
                      形状.点击时字号 = 形状.字号;
            ...
            // 检查是否点击在整体交互框内（拖拽）
            // 优先检查：如果Shift已按下，且鼠标在多选内的编组内，则只拖拽该编组
            // 注意：只有当"多选内包含编组且不是整个多选就是一个编组"时，才设置拖拽编组ID
            // 如果整个多选就是一个编组，应该允许单独移动编组内的形状
            if (this.鼠标位于多选整体交互框内()) {
              // 如果Shift已按下，检查是否在多选内的编组内
              if (this.全局标志.Shift已按下) {
                const 多选内包含编组 = this.全局属性.多选形状组.some((形状) => this.形状已编组(形状));
                if (多选内包含编组) {
                  // 检查整个多选是否就是一个编组
                  const 多选数量 = this.全局属性.多选形状组.length;
                  const 所有形状属于同一编组 =
                  ...
                  // 只有当"多选内包含编组且不是整个多选就是一个编组"时，才设置拖拽编组ID
                  if (!所有形状属于同一编组) {
                    // 检查鼠标是否在某个编组内
                    const 已处理的组ID集合 = new Set();
                    for (const 形状 of this.全局属性.多选形状组) {
              ...
              if (交互框尺寸) {
                this.全局属性.初始交互框尺寸 = { ...交互框尺寸 };
                // 使用扩展后的边界计算中心位置（与绘制多选整体交互框的逻辑一致）
                let 边界 = this.获取多选形状的边界();
                if (边界) {
                  边界 = this.计算多选扩展边界(边界);
                  if (边界) {
                    this.全局属性.初始交互框位置 = { x: 边界.centerX, y: 边界.centerY };
  ...
  添加canvas鼠标抬起事件() {
    this.canvas.addEventListener("mouseup", (e) => {
      ...
      if (this.全局标志.正在框选) {
        ...
        // 处理编组：只选中完全包含的编组，不选中部分包含的编组内的形状
        const 已处理的组ID集合 = new Set();
        const 完全包含的组ID集合 = new Set(); // 记录完全包含的编组ID
        ...
        for (const 形状 of 框选内的形状) {
          if (this.形状已编组(形状)) {
            ...
            if (!已处理的组ID集合.has(组ID)) {
              ...
              if (组内形状 && 组内形状.length >= 2) {
                // 检查框选是否完全包含该编组的所有形状
                const 完全包含 = 组内形状.every((组内形状对象) => 框选内的形状.includes(组内形状对象));
                if (完全包含) {
                  // 完全包含：选中整个编组
                  完全包含的组ID集合.add(组ID);
                  组内形状.forEach((组内形状对象) => {
        ...
        框选内的形状.forEach((形状) => {
          if (!this.形状已编组(形状)) {
            this.添加到多选组(形状);
          } else if (!完全包含的组ID集合.has(形状.所属组ID)) {
            // 部分包含的编组内的形状，不添加到多选组
            // 完全包含的编组内的形状已经在上面添加过了
          }
        });
          ...
          this.全局属性.多选缩放初始位置组.forEach((初始位置) => {
            ...
            if (形状.形状 === "圆" || 形状.形状 === "多边形" || 形状.形状 === "多角星") {
              ...
              缩放操作数组.push({
                ...
                旋转弧度: 初始位置.按下时旋转弧度 !== undefined ? structuredClone(初始位置.按下时旋转弧度) : undefined,
                顶点坐标组: 初始位置.按下时顶点坐标组 ? [...初始位置.按下时顶点坐标组] : [],
                缩放比例: 初始位置.按下时缩放比例 ? { ...初始位置.按下时缩放比例 } : { x: 1, y: 1 },
              });
            } else if (
              形状.形状 === "直线" ||
              形状.形状 === "自由" ||
              ...
              if (形状.形状 === "圆") {
                ...
                if (初始位置.按下时顶点坐标组 && 初始位置.按下时顶点坐标组.length > 0) {
                  移动操作数组.push({
                    ...
                    顶点坐标组: [...初始位置.按下时顶点坐标组],
                    旋转弧度:
                      初始位置.按下时旋转弧度 !== undefined ? structuredClone(初始位置.按下时旋转弧度) : undefined,
                  });
                } else {
                  // 未旋转的文本，只保存坐标
                  移动操作数组.push({
      ...
      if (this.交互框 && 形状移动距离 > 0) {
        if (this.全局属性.选中形状.形状 === "圆") {
          ...
          // 对于文本，如果旋转过，应该保存顶点坐标组（与多选移动一致）
          if (this.全局属性.选中形状.点击时顶点坐标组 && this.全局属性.选中形状.点击时顶点坐标组.length > 0) {
            this.数据集.操作记录.push({
              操作类型: "移动",
              形状: this.全局属性.选中形状,
              顶点坐标组: [...this.全局属性.选中形状.点击时顶点坐标组],
              旋转弧度:
                this.全局属性.选中形状.点击时旋转弧度 !== undefined
                  ? structuredClone(this.全局属性.选中形状.点击时旋转弧度)
                  : undefined,
            });
          } else {
            // 未旋转的文本，只保存坐标
      ...
      if (this.全局属性.选中形状 && this.全局属性.选中形状.点击时尺寸) {
        ...
        if (已缩放) {
          this.数据集.操作记录.push({
            ...
            缩放前缩放比例: this.全局属性.选中形状.点击时缩放比例
              ? { ...this.全局属性.选中形状.点击时缩放比例 }
              : { x: 1, y: 1 },
          });
        }
  ...
  清理所有拖拽状态() {
    ...
    if (this.全局标志.Alt拖拽复制中 && this.全局属性.复制预览组) {
      // 复制组的情况
      const 偏移量 = {
        x: this.全局属性.鼠标坐标 ? this.全局属性.鼠标坐标.x - this.全局属性.复制预览组[0].初始鼠标位置.x : 0,
        y: this.全局属性.鼠标坐标 ? this.全局属性.鼠标坐标.y - this.全局属性.复制预览组[0].初始鼠标位置.y : 0,
      };
      this.复制组(this.全局属性.复制源组, 偏移量);
      this.全局属性.复制预览组.forEach((预览形状) => {
        delete 预览形状.初始鼠标位置;
      ...
      this.全局属性.复制源组 = null;
      this.撤销按钮.classList.remove("禁用");
    } else if (this.全局标志.Alt拖拽复制中 && this.全局属性.复制预览形状) {
      // 单个形状复制的情况
      delete this.全局属性.复制预览形状.初始鼠标位置;
      this.数据集.基础形状对象组.push(this.全局属性.复制预览形状);
      this.全局属性.复制预览形状 = null;
    ...
    // 2. 处理多选旋转结束（完成旋转，但不记录操作历史）
    if (this.全局标志.多选旋转中 && this.全局属性.多选形状组.length > 0) {
      // 保存旋转状态到映射中（参考鼠标抬起事件的逻辑）
      const 是编组 =
        this.全局属性.多选形状组.length > 0 &&
        this.全局属性.多选形状组.every((形状) => 形状.所属组ID === this.全局属性.多选形状组[0].所属组ID) &&
        this.全局属性.多选形状组[0].所属组ID !== undefined;
      ...
      if (是编组) {
        const 组ID = this.全局属性.多选形状组[0].所属组ID;
        const 之前的旋转弧度 = this.全局属性.编组旋转弧度映射[组ID] || 0;
        const 增量 = this.全局属性.多选整体旋转弧度 || 0;
        const 标准化增量 = ((增量 % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
        this.全局属性.编组旋转弧度映射[组ID] = 之前的旋转弧度 + 标准化增量;
        let 标准化旋转 = this.全局属性.编组旋转弧度映射[组ID];
        while (标准化旋转 < 0) 标准化旋转 += Math.PI * 2;
        while (标准化旋转 >= Math.PI * 2) 标准化旋转 -= Math.PI * 2;
        this.全局属性.编组旋转弧度映射[组ID] = 标准化旋转;
        if (this.全局属性.多选旋转初始边界 && !this.全局属性.编组旋转边界映射[组ID]) {
          this.全局属性.编组旋转边界映射[组ID] = structuredClone(this.全局属性.多选旋转初始边界);
        }
    ...
    // 4. 处理多选拖拽结束（完成拖拽，但不记录操作历史）
    if (this.全局标志.多选拖拽中 && this.全局属性.多选形状组.length > 0) {
      this.全局标志.多选拖拽中 = false;
      this.全局属性.多选初始位置组 = [];
      this.全局标志.Shift单独拖拽中 = false;
      this.全局属性.初始交互框尺寸 = null;
      this.全局属性.初始交互框位置 = null;
      // 清除拖拽编组ID
      this.全局属性.拖拽编组ID = null;
      if (this.全局属性.鼠标坐标 && this.全局属性.点击坐标) {
        const 鼠标移动距离 = Math.sqrt(
          Math.abs(this.全局属性.鼠标坐标.x - this.全局属性.点击坐标.x) ** 2 +
            Math.abs(this.全局属性.鼠标坐标.y - this.全局属性.点击坐标.y) ** 2,
        );
    ...
    if (this.全局标志.正在框选) {
      ...
      // 清除框选预览编组ID集合
      this.全局属性.框选预览编组ID集合.clear();
      if (this.全局属性.框选起点 && this.全局属性.框选终点) {
        const minX = Math.min(this.全局属性.框选起点.x, this.全局属性.框选终点.x);
        const minY = Math.min(this.全局属性.框选起点.y, this.全局属性.框选终点.y);
        const maxX = Math.max(this.全局属性.框选起点.x, this.全局属性.框选终点.x);
        const maxY = Math.max(this.全局属性.框选起点.y, this.全局属性.框选终点.y);
    ...
    // 6. 处理单个形状的拖拽/旋转/缩放结束
    if (this.全局标志.缩放中 && this.全局属性.选中形状) {
      if (this.全局属性.选中形状.形状 === "矩形" || this.全局属性.选中形状.形状 === "图像") {
        this.规范化矩形顶点顺序(this.全局属性.选中形状);
      }
    }
    ...
    if (
      ...
      if (this.当前形状对象.顶点坐标组 && this.当前形状对象.顶点坐标组.length > 0) {
        const path = this.当前形状对象.路径;
        this.当前形状对象.路径 = null;
        const 克隆 = structuredClone(this.当前形状对象);
        克隆.路径 = path;
  ...
  限制移动方向(偏移量, 限制为水平或垂直) {
    ...
    // 如果Ctrl键按下，限制为水平或垂直移动（选择距离更近的方向）
    const 水平距离 = Math.abs(偏移量.x);
    const 垂直距离 = Math.abs(偏移量.y);
    if (水平距离 > 垂直距离) {
      // 水平移动距离更大，限制为水平移动
      return { x: 偏移量.x, y: 0 };
    } else {
  ...
  添加键盘事件() {
    document.addEventListener("keydown", (e) => {
      ...
      if (this.全局标志.文本编辑中) {
        if (e.key === "Escape") {
          e.preventDefault();
          // 只有在文本工具添加文本时（没有双击待编组形状）才记录ESC按下时间
          if (!this.全局属性.双击待编组形状 && !this.全局属性.ESC按下时间) {
            this.全局属性.ESC按下时间 = Date.now();
            // 使用requestAnimationFrame检测是否超过500ms
            const 检测ESC按下时间 = () => {
              if (!this.全局属性.ESC按下时间 || !this.全局标志.文本编辑中 || this.全局属性.双击待编组形状) {
                return; // ESC已释放或不在文本编辑模式或双击添加，停止检测
              }
              const 经过时间 = Date.now() - this.全局属性.ESC按下时间;
              if (经过时间 >= 500 && !this.全局属性.文本背景矩形) {
                // 超过500ms且还没创建矩形，创建矩形并自动确认文本
                this.创建文本背景矩形();
                // 设置标志，防止ESC后续的取消选择功能
                this.全局标志.ESC确认文本 = true;
                // 自动确认文本（无需等待keyup）
                this.退出文本编辑模式();
              } else if (经过时间 < 500 && !this.全局属性.文本背景矩形) {
                // 未超过500ms且还没创建矩形，继续检测
                requestAnimationFrame(检测ESC按下时间);
              }
            };
            requestAnimationFrame(检测ESC按下时间);
          }
          // 如果是双击添加文本，保持原有逻辑（立即确认）
          if (this.全局属性.双击待编组形状) {
            // 设置标志，防止ESC后续的取消选择功能
            this.全局标志.ESC确认文本 = true;
            this.退出文本编辑模式();
          }
      ...
      // ESC键：取消选择（单选或多选）
      // 如果刚刚通过ESC确认了文本，忽略取消选择功能
      if (e.key === "Escape" && !this.全局标志.ESC确认文本) {
        // 如果有多选，清空多选组
        if (this.全局属性.多选形状组.length > 0) {
          this.清空多选组();
          this.交互框 = null;
      ...
      if (e.key === "Shift") {
        ...
        // 如果多选内包含编组，且鼠标位于编组内，清除编组内所有形状的悬停状态
        // 注意：只有当"多选内包含编组且不是整个多选就是一个编组"时，才清除编组内形状的悬停状态
        // 如果整个多选就是一个编组，应该允许单独移动编组内的形状
        if (this.全局属性.多选形状组.length > 0) {
          const 多选内包含编组 = this.全局属性.多选形状组.some((形状) => this.形状已编组(形状));
          if (多选内包含编组) {
            ...
            // 只有当"多选内包含编组且不是整个多选就是一个编组"时，才清除编组内形状的悬停状态
            if (!所有形状属于同一编组) {
              // 检查鼠标是否在某个编组内
              const 已处理的组ID集合 = new Set();
              for (const 形状 of this.全局属性.多选形状组) {
    ...
    document.addEventListener("keyup", (e) => {
      // 文本编辑模式：ESC keyup确认文本（仅限文本工具添加，且500ms内松开）
      if (this.全局标志.文本编辑中 && e.key === "Escape") {
        // 只有在文本工具添加文本时（没有双击待编组形状）才处理ESC keyup
        // 如果已经有背景矩形，说明已经自动确认了，不需要再处理
        if (!this.全局属性.双击待编组形状 && this.全局属性.ESC按下时间 && !this.全局属性.文本背景矩形) {
          // 清除ESC按下时间
          this.全局属性.ESC按下时间 = null;
          // 设置标志，防止ESC后续的取消选择功能
          this.全局标志.ESC确认文本 = true;
          // 确认文本（500ms内松开，没有背景矩形）
          this.退出文本编辑模式();
        }
        return;
      }

      // 清除ESC确认文本标志（ESC keyup时）
      if (e.key === "Escape") {
        this.全局标志.ESC确认文本 = false;
      }
  ...
  退出文本编辑模式() {
    ...
    // 如果文本为空且是文本工具添加（不是编辑已有文本），清除ESC相关状态
      ...
      if (this.全局属性.当前编辑文本形状) {
        ...
        if (this.全局属性.双击待编组形状 && 新文本形状 && this.全局属性.双击待编组形状中心) {
          ...
          if (双击的形状.所属组ID !== undefined) {
            ...
            // 将该组标记为只能等比缩放
            if (!this.数据集.组属性[最终组ID]) {
              this.数据集.组属性[最终组ID] = {};
            }
            this.数据集.组属性[最终组ID].只能等比缩放 = true;
            ...
            // 如果不在组中，创建新组（标记为只能等比缩放）
            最终组ID = this.创建组([双击的形状, 新文本形状], true);
          ...
          // 文本工具添加文本：如果有背景矩形，需要编组
          if (this.全局属性.文本背景矩形 && 新文本形状) {
            // 重新计算矩形位置（因为文本位置可能已经更新）
            // 文本中心点 = (实际x + 文本宽度/2, 实际y)
            const 文本中心x = 新文本形状.坐标.x + 新文本形状.尺寸.宽 / 2;
            const 文本中心y = 新文本形状.坐标.y;

            // 更新矩形位置使其中心点与文本中心点一致
            const 矩形宽 = this.全局属性.文本背景矩形.尺寸.宽;
            const 矩形高 = this.全局属性.文本背景矩形.尺寸.高;
            this.全局属性.文本背景矩形.坐标.x = 文本中心x - 矩形宽 / 2;
            this.全局属性.文本背景矩形.坐标.y = 文本中心y - 矩形高 / 2;

            // 更新矩形的顶点坐标组
            this.全局属性.文本背景矩形.顶点坐标组 = [
              { x: this.全局属性.文本背景矩形.坐标.x, y: this.全局属性.文本背景矩形.坐标.y },
              { x: this.全局属性.文本背景矩形.坐标.x + 矩形宽, y: this.全局属性.文本背景矩形.坐标.y },
              { x: this.全局属性.文本背景矩形.坐标.x + 矩形宽, y: this.全局属性.文本背景矩形.坐标.y + 矩形高 },
              { x: this.全局属性.文本背景矩形.坐标.x, y: this.全局属性.文本背景矩形.坐标.y + 矩形高 },
            ];

            // 重新计算极值坐标和路径
            this.全局属性.文本背景矩形.极值坐标 = this.获取极值坐标(this.全局属性.文本背景矩形);
            this.更新路径(this.全局属性.文本背景矩形);

            // 将矩形添加到基础形状对象组（索引在文本前面）
            const 文本索引 = this.数据集.基础形状对象组.indexOf(新文本形状);
            if (文本索引 !== -1) {
              this.数据集.基础形状对象组.splice(文本索引, 0, this.全局属性.文本背景矩形);
            } else {
              this.数据集.基础形状对象组.unshift(this.全局属性.文本背景矩形);
            }

            // 创建编组（矩形和文本，标记为只能等比缩放）
            const 组ID = this.创建组([this.全局属性.文本背景矩形, 新文本形状], true);

            // 记录操作：添加文本和背景矩形并编组
            this.数据集.操作记录.push({
              操作类型: "添加文本和背景矩形并编组",
              文本形状: 新文本形状,
              矩形形状: this.全局属性.文本背景矩形,
              组ID: 组ID,
            });
  ...
  创建文本背景矩形() {
    ...
    // 计算矩形的尺寸（文本上下左右各加10）
    const 矩形宽 = 临时文本形状.尺寸.宽 + 20;
    const 矩形高 = 临时文本形状.尺寸.高 + 16;

    // 计算矩形的坐标（矩形中心点 = 文本中心点）
    // 文本中心点 = (实际x + 文本宽度/2, 实际y)
    // 矩形左上角 = (文本中心.x - 矩形宽度/2, 文本中心.y - 矩形高度/2)
    const 文本中心x = 实际x + 临时文本形状.尺寸.宽 / 2;
    const 文本中心y = 实际y;
    const 矩形x = 文本中心x - 矩形宽 / 2;
    const 矩形y = 文本中心y - 矩形高 / 2;
    ...
    const 矩形形状 = {
      ...
      坐标: { x: 矩形x, y: 矩形y },
      尺寸: { 宽: 矩形宽, 高: 矩形高 },
      描边色: this.全局属性.描边色, // 使用全局描边色
      填充色: this.全局属性.填充色, // 使用全局填充色
      描边宽度: this.全局属性.描边宽度,
      圆角: 0,
      已选中: false,
      已悬停: false,
    ...
    // 计算极值坐标
    矩形形状.极值坐标 = this.获取极值坐标(矩形形状);

    // 更新路径
    this.更新路径(矩形形状);

    // 保存矩形形状引用
    this.全局属性.文本背景矩形 = 矩形形状;
  ...
  绘制文本(文本形状) {
    ...
    // 计算中心点
    // 使用极值坐标的中心点，这是旋转后的几何中心
    let 中心x, 中心y;
    if (文本形状.极值坐标 && 文本形状.极值坐标.中心) {
      中心x = 文本形状.极值坐标.中心.x;
      中心y = 文本形状.极值坐标.中心.y;
    } else {
      // 如果没有极值坐标，使用坐标计算（未旋转时）
      中心x = 文本形状.坐标.x + 文本形状.尺寸.宽 / 2;
      中心y = 文本形状.坐标.y;
    }

    // 应用变换：先平移至中心，然后旋转，再缩放
    // 变换后的坐标系原点在中心点，已经旋转和缩放
    ...
    // 绘制文本（支持多行）
    // 在变换后的坐标系中，左边中心点相对于中心点的偏移是 (-尺寸.宽/2, 0)
    const 行数组 = 文本形状.文本内容.split("\n");
    const 行高 = 文本形状.字号 * 1.2; // 行高为字号的1.2倍
    const 总高度 = 行数组.length * 行高;

    // 左边中心点相对于中心点的偏移（在局部坐标系中）
    const 左边中心偏移x = -文本形状.尺寸.宽 / 2;
    const 左边中心偏移y = 0;

    // 第一行的y相对于中心点的偏移
    const 第一行偏移y = -总高度 / 2 + 行高 / 2;

    行数组.forEach((行, 索引) => {
      const y偏移 = 第一行偏移y + 索引 * 行高;
      this.ctx.fillText(行, 左边中心偏移x, y偏移);
    });
  ...
  获取边界缩放锚点(鼠标位于边界, 形状对象) {
    ...
    if (形状对象.形状 === "圆" || 形状对象.形状 === "多边形" || 形状对象.形状 === "多角星") {
      ...
      // 🎯 对于图像和文本，使用局部坐标系动态查找对边的两个顶点（支持旋转）
      if (!形状对象.顶点坐标组 || 形状对象.顶点坐标组.length < 4) return null;
  ...
  执行缩放(形状对象, 等比) {
    ...
    // 对于旋转后的圆形、多边形、多角星、矩形、图像、箭头、文本，需要在局部坐标系中计算缩放比例
    const 需要局部坐标系 =
      (形状对象.形状 === "圆" ||
        形状对象.形状 === "多边形" ||
        形状对象.形状 === "多角星" ||
    ...
    if (需要局部坐标系) {
      ...
      // 在局部坐标系中计算缩放比例
      // 对于基于尺寸缩放的形状，使用基于初始尺寸的比例缩放（解决小形状缩放不匹配问题）
      let 初始尺寸 = null;
      if (基于尺寸缩放) {
        if (
          (形状对象.形状 === "矩形" || 形状对象.形状 === "图像") &&
          形状对象.点击时顶点坐标组 &&
          形状对象.点击时顶点坐标组.length === 4
        ) {
          // 矩形和图像：计算初始尺寸（在局部坐标系中）
          const 初始顶点局部 = 形状对象.点击时顶点坐标组.map((v) => {
            const dx = v.x - 缩放锚点.x;
            const dy = v.y - 缩放锚点.y;
            return {
              x: dx * cos - dy * sin,
          ...
          const 初始最左 = Math.min(...初始顶点局部.map((v) => v.x));
          const 初始最右 = Math.max(...初始顶点局部.map((v) => v.x));
          const 初始最上 = Math.min(...初始顶点局部.map((v) => v.y));
          const 初始最下 = Math.max(...初始顶点局部.map((v) => v.y));
          初始尺寸 = {
            宽: 初始最右 - 初始最左,
            高: 初始最下 - 初始最上,
          };
        } else if (
          (形状对象.形状 === "直线" || 形状对象.形状 === "自由") &&
          形状对象.点击时顶点坐标组 &&
          形状对象.点击时顶点坐标组.length > 0
        ) {
          // 直线和自由路径：计算初始尺寸（在局部坐标系中）
          const 初始顶点局部 = 形状对象.点击时顶点坐标组.map((v) => {
            const dx = v.x - 缩放锚点.x;
            const dy = v.y - 缩放锚点.y;
            return {
        ...
        } else if (形状对象.形状 === "圆" && 形状对象.点击时尺寸) {
          // 圆：使用半径作为初始尺寸
          初始尺寸 = {
            宽: 形状对象.点击时尺寸.水平半径 * 2,
            高: 形状对象.点击时尺寸.垂直半径 * 2,
          };
        } else if (形状对象.形状 === "多边形" && 形状对象.点击时尺寸) {
          // 多边形：使用半径作为初始尺寸
          初始尺寸 = {
            宽: 形状对象.点击时尺寸.水平半径 * 2,
            高: 形状对象.点击时尺寸.垂直半径 * 2,
          };
        } else if (形状对象.形状 === "多角星" && 形状对象.点击时尺寸) {
          // 多角星：使用外半径作为初始尺寸
          初始尺寸 = {
            宽: 形状对象.点击时尺寸.外半径.水平 * 2,
            高: 形状对象.点击时尺寸.外半径.垂直 * 2,
          };
        } else if (形状对象.形状 === "箭头" && 形状对象.点击时顶点坐标组 && 形状对象.点击时顶点坐标组.length >= 2) {
          // 箭头：计算初始长度和边界框（在局部坐标系中）
          const 初始起点 = 形状对象.点击时顶点坐标组[0];
          const 初始终点 = 形状对象.点击时顶点坐标组[1];
          const dx = 初始终点.x - 初始起点.x;
          const dy = 初始终点.y - 初始起点.y;
          const 初始长度 = Math.sqrt(dx * dx + dy * dy);
        ...
        } else if (形状对象.形状 === "文本" && 形状对象.点击时顶点坐标组 && 形状对象.点击时顶点坐标组.length > 0) {
          // 文本：计算初始尺寸（在局部坐标系中）
          const 初始顶点局部 = 形状对象.点击时顶点坐标组.map((v) => {
            const dx = v.x - 缩放锚点.x;
            const dy = v.y - 缩放锚点.y;
      ...
      if (缩放模式 === "horizontal") {
        if (基于尺寸缩放 && 初始尺寸 && 初始尺寸.宽 > 阈值) {
          // 基于初始尺寸的比例缩放
          const 初始距离 = Math.abs(点击_局部x);
          const 当前距离 = Math.abs(鼠标_局部x);
          const 距离变化 = 当前距离 - 初始距离;
          水平缩放比例 = 1 + 距离变化 / 初始尺寸.宽;
          if (点击_局部x * 鼠标_局部x < 0) 水平缩放比例 = -水平缩放比例;
        } else {
          水平缩放比例 = Math.abs(点击_局部x) > 阈值 ? 鼠标_局部x / 点击_局部x : 1;
        }
      } else if (缩放模式 === "vertical") {
        if (基于尺寸缩放 && 初始尺寸 && 初始尺寸.高 > 阈值) {
          // 基于初始尺寸的比例缩放
          const 初始距离 = Math.abs(点击_局部y);
          const 当前距离 = Math.abs(鼠标_局部y);
          const 距离变化 = 当前距离 - 初始距离;
          垂直缩放比例 = 1 + 距离变化 / 初始尺寸.高;
          if (点击_局部y * 鼠标_局部y < 0) 垂直缩放比例 = -垂直缩放比例;
        } else {
        ...
        if (基于尺寸缩放 && 初始尺寸) {
          // 基于初始尺寸的比例缩放
          if (初始尺寸.宽 > 阈值) {
            const 初始距离X = Math.abs(点击_局部x);
            const 当前距离X = Math.abs(鼠标_局部x);
            const 距离变化X = 当前距离X - 初始距离X;
            水平缩放比例 = 1 + 距离变化X / 初始尺寸.宽;
            if (点击_局部x * 鼠标_局部x < 0) 水平缩放比例 = -水平缩放比例;
          } else {
            水平缩放比例 = Math.abs(点击_局部x) > 阈值 ? 鼠标_局部x / 点击_局部x : 1;
          }
          if (初始尺寸.高 > 阈值) {
            const 初始距离Y = Math.abs(点击_局部y);
            const 当前距离Y = Math.abs(鼠标_局部y);
            const 距离变化Y = 当前距离Y - 初始距离Y;
            垂直缩放比例 = 1 + 距离变化Y / 初始尺寸.高;
            if (点击_局部y * 鼠标_局部y < 0) 垂直缩放比例 = -垂直缩放比例;
          } else {
            垂直缩放比例 = Math.abs(点击_局部y) > 阈值 ? 鼠标_局部y / 点击_局部y : 1;
          }
      ...
      if (基于尺寸缩放) {
        if (
          ...
          // 矩形和图像：计算初始尺寸（在世界坐标系中）
          const 初始顶点 = 形状对象.点击时顶点坐标组;
          const 初始最左 = Math.min(...初始顶点.map((v) => v.x));
          const 初始最右 = Math.max(...初始顶点.map((v) => v.x));
          const 初始最上 = Math.min(...初始顶点.map((v) => v.y));
          const 初始最下 = Math.max(...初始顶点.map((v) => v.y));
          初始尺寸 = {
            宽: 初始最右 - 初始最左,
            高: 初始最下 - 初始最上,
          };
      ...
      if (缩放模式 === "horizontal") {
        ...
        if (基于尺寸缩放 && 初始尺寸 && 初始尺寸.宽 > 阈值) {
          // 基于初始尺寸的比例缩放（使用绝对距离，与局部坐标系一致，保证左上/左边界正确）
          const 初始距离 = Math.abs(初始水平距离);
          const 当前距离 = Math.abs(当前水平距离);
          const 距离变化 = 当前距离 - 初始距离;
          水平缩放比例 = 1 + 距离变化 / 初始尺寸.宽;
          if (初始水平距离 * 当前水平距离 < 0) 水平缩放比例 = -水平缩放比例;
        } else {
        ...
        if (基于尺寸缩放 && 初始尺寸 && 初始尺寸.高 > 阈值) {
          // 基于初始尺寸的比例缩放（使用绝对距离，与局部坐标系一致，保证左上/上边界正确）
          const 初始距离 = Math.abs(初始垂直距离);
          const 当前距离 = Math.abs(当前垂直距离);
          const 距离变化 = 当前距离 - 初始距离;
          垂直缩放比例 = 1 + 距离变化 / 初始尺寸.高;
          if (初始垂直距离 * 当前垂直距离 < 0) 垂直缩放比例 = -垂直缩放比例;
        } else {
  ...
  应用缩放(形状对象, 水平缩放比例, 垂直缩放比例, 缩放锚点) {
    if (形状对象.形状 === "文本") {
      // 文本形状：等比缩放，通过修改字号实现，支持镜像翻转
      // 检测是否发生水平或垂直翻转（缩放比例为负数）
      const 水平翻转 = 水平缩放比例 < 0;
      const 垂直翻转 = 垂直缩放比例 < 0;

      // 计算等比缩放比例（取两个方向的绝对值的平均值）
      const 原始缩放比例 = (Math.abs(水平缩放比例) + Math.abs(垂直缩放比例)) / 2;

      // 计算目标字号（使用浮点数，增加平滑度）
      const 目标字号 = 形状对象.点击时字号 * 原始缩放比例;

      // 限制字号范围：最小12px，最大100px
      形状对象.字号 = Math.max(12, Math.min(500, 目标字号));
      ...
      if (!形状对象.点击时缩放比例) {
        形状对象.点击时缩放比例 = { x: 1, y: 1 };
      }

      // 根据翻转状态更新缩放比例
      // 如果发生水平翻转，缩放比例x取反；如果发生垂直翻转，缩放比例y取反
      if (水平翻转) {
        形状对象.缩放比例.x = -形状对象.点击时缩放比例.x;
      } else {
        形状对象.缩放比例.x = 形状对象.点击时缩放比例.x;
      }
      if (垂直翻转) {
        形状对象.缩放比例.y = -形状对象.点击时缩放比例.y;
      } else {
        形状对象.缩放比例.y = 形状对象.点击时缩放比例.y;
      }

      // 缩放顶点坐标（使用实际缩放比例，支持镜像翻转）
      if (形状对象.点击时顶点坐标组 && 形状对象.顶点坐标组) {
        if (形状对象.旋转弧度) {
          ...
          for (let i = 0; i < 形状对象.顶点坐标组.length; i++) {
            ...
            // 在局部坐标系中应用缩放比例（包括镜像翻转）
            // 如果发生水平翻转，局部x取反；如果发生垂直翻转，局部y取反
            let 缩放后局部x = 局部x * 实际缩放比例;
            let 缩放后局部y = 局部y * 实际缩放比例;
            if (水平翻转) {
              缩放后局部x = -缩放后局部x;
            }
            if (垂直翻转) {
          ...
          // 对于未旋转的文本，直接在世界坐标系中缩放（包括镜像翻转）
          for (let i = 0; i < 形状对象.顶点坐标组.length; i++) {
            let dx = 形状对象.点击时顶点坐标组[i].x - 缩放锚点.x;
            let dy = 形状对象.点击时顶点坐标组[i].y - 缩放锚点.y;
            ...
            形状对象.顶点坐标组[i].x = 缩放锚点.x + dx;
            形状对象.顶点坐标组[i].y = 缩放锚点.y + dy;
          }
        }
      }
      ...
      // 从顶点坐标组计算实际宽度（与多选旋转和单独文本形状移动一致）
      let 实际宽度;
      if (形状对象.旋转弧度) {
        const 旋转弧度 = 形状对象.旋转弧度;
        const cos = Math.cos(-旋转弧度);
        const sin = Math.sin(-旋转弧度);
        const 局部顶点 = 形状对象.顶点坐标组.map((v) => {
          const dx = v.x - 中心x;
          const dy = v.y - 中心y;
          return {
            局部x: dx * cos - dy * sin,
        ...
        实际宽度 = Math.max(...形状对象.顶点坐标组.map((v) => v.x)) - Math.min(...形状对象.顶点坐标组.map((v) => v.x));
      }
      ...
      if (形状对象.旋转弧度) {
        ...
        const cos = Math.cos(旋转弧度);
        const sin = Math.sin(旋转弧度);
        形状对象.坐标.x = 中心x - 半宽 * cos;
        形状对象.坐标.y = 中心y - 半宽 * sin;
      } else {
        形状对象.坐标.x = 中心x - 半宽;
        形状对象.坐标.y = 中心y;
      }
  ...
  执行多选缩放() {
    ...
    // 检查是否有任何形状属于只能等比缩放的组
    const 有只能等比缩放的组 = this.全局属性.多选形状组.some((形状) => {
      if (形状.所属组ID !== undefined && this.数据集.组属性[形状.所属组ID]) {
        return this.数据集.组属性[形状.所属组ID].只能等比缩放 === true;
      }
      return false;
    });
    ...
    // 否则强制使用等比缩放
    // 如果有形状属于只能等比缩放的组，强制使用等比缩放
    const 强制等比缩放 = 有旋转 || this.全局属性.缩放模式 === "proportional" || 有只能等比缩放的组;
    ...
    // 检查文本形状的限制，调整缩放比例（参考单个文本形状的缩放逻辑）
    // 文本形状使用等比缩放比例：(Math.abs(水平缩放比例) + Math.abs(垂直缩放比例)) / 2
    const 原始等比缩放比例 = (Math.abs(水平缩放比例) + Math.abs(垂直缩放比例)) / 2;

    // 找到最限制性的缩放比例
    let 最终等比缩放比例 = 原始等比缩放比例;
    let 有文本限制 = false;

    for (const 初始位置 of this.全局属性.多选缩放初始位置组) {
      const 形状 = 初始位置.形状;
      if (形状.形状 === "文本" && 初始位置.按下时字号 !== undefined) {
        const 初始字号 = 初始位置.按下时字号;
        // 计算目标字号（参考单个文本形状的逻辑）
        const 目标字号 = Math.round(初始字号 * 原始等比缩放比例);

        // 限制字号范围：最小12px，最大100px（参考单个文本形状的逻辑）
        const 限制后字号 = Math.max(12, Math.min(100, 目标字号));

        // 计算实际应用的缩放比例（考虑字号限制）
        const 实际缩放比例 = 限制后字号 / 初始字号;

        // 如果字号被限制了，说明达到了限制，需要调整缩放比例
        if (目标字号 !== 限制后字号) {
          有文本限制 = true;
          // 选择最限制性的缩放比例（放大时取最小值，缩小时取最大值）
          if (原始等比缩放比例 > 1) {
            // 放大时，取最小值（最严格的限制）
            最终等比缩放比例 = Math.min(最终等比缩放比例, 实际缩放比例);
          } else {
            // 缩小时，取最大值（最严格的限制）
            最终等比缩放比例 = Math.max(最终等比缩放比例, 实际缩放比例);
          }
        }
      }
    }

    // 如果有文本限制，调整水平缩放比例和垂直缩放比例
    if (有文本限制 && 最终等比缩放比例 !== 原始等比缩放比例) {
      const 缩放比例调整系数 = 最终等比缩放比例 / 原始等比缩放比例;
      ...
      // 调整缩放比例，保持方向和翻转状态
      const 调整后绝对值 = Math.abs(水平缩放比例) * 缩放比例调整系数;
      水平缩放比例 = 水平翻转 ? -调整后绝对值 : 调整后绝对值;

      const 调整后垂直绝对值 = Math.abs(垂直缩放比例) * 缩放比例调整系数;
      垂直缩放比例 = 垂直翻转 ? -调整后垂直绝对值 : 调整后垂直绝对值;
    }
  ...
  应用多选形状缩放(形状, 初始位置, 水平缩放比例, 垂直缩放比例, 缩放锚点, 初始边界) {
    ...
    // 对于文本，应用缩放已经正确处理了顶点坐标组和坐标的更新，不需要再调用更新文本形状
    // 因为更新文本形状会基于坐标重新计算顶点坐标组，这会破坏应用缩放中已经正确计算的顶点坐标组
    // if (形状.形状 === "文本") {
    //   this.更新文本形状(形状);
    // }
  }
  ...
  水平翻转形状(形状) {
    ...
    if (形状.形状 === "圆") {
      ...
      // 保存当前的顶点坐标组（翻转前）
      const 翻转前顶点坐标组 = 形状.顶点坐标组 ? 形状.顶点坐标组.map((v) => ({ ...v })) : null;
      const 翻转前旋转弧度 = 形状.旋转弧度 !== undefined ? 形状.旋转弧度 : 0;
      ...
      // 如果有顶点坐标组，围绕中心点翻转顶点坐标组
      if (翻转前顶点坐标组 && 翻转前顶点坐标组.length === 4) {
        // 围绕中心点水平翻转顶点坐标组
        形状.顶点坐标组 = 翻转前顶点坐标组.map((v) => ({
          x: 中心点.x - (v.x - 中心点.x),
          y: v.y,
        }));

        // 从翻转后的顶点坐标组反推坐标
        const 中心x = (形状.顶点坐标组[0].x + 形状.顶点坐标组[1].x + 形状.顶点坐标组[2].x + 形状.顶点坐标组[3].x) / 4;
        const 中心y = (形状.顶点坐标组[0].y + 形状.顶点坐标组[1].y + 形状.顶点坐标组[2].y + 形状.顶点坐标组[3].y) / 4;

        // 从顶点坐标组计算实际宽度（使用翻转后的旋转角度）
        let 实际宽度;
        if (形状.旋转弧度) {
          const 旋转弧度 = 形状.旋转弧度;
          const cos = Math.cos(-旋转弧度);
      ...
      if (形状.顶点坐标组 && 形状.顶点坐标组.length > 0) {
        ...
        // 过滤掉无效的顶点，然后翻转
        形状.顶点坐标组 = 形状.顶点坐标组
          .filter((坐标) => 坐标 && 坐标.x !== undefined && 坐标.y !== undefined)
          .map((坐标) => ({
            x: 中心点.x - (坐标.x - 中心点.x),
            y: 坐标.y,
          }));

        // 如果过滤后顶点数量不对，尝试从极值坐标重新生成
        if (形状.形状 === "矩形" && 形状.顶点坐标组.length !== 4) {
          if (形状.极值坐标 && 形状.尺寸) {
            const 中心x = (形状.极值坐标.左 + 形状.极值坐标.右) / 2;
            const 中心y = (形状.极值坐标.上 + 形状.极值坐标.下) / 2;
            const 半宽 = (形状.极值坐标.右 - 形状.极值坐标.左) / 2;
            const 半高 = (形状.极值坐标.下 - 形状.极值坐标.上) / 2;
            ...
            形状.顶点坐标组 = [
              { x: 中心x - 半宽 * cos + 半高 * sin, y: 中心y - 半宽 * sin - 半高 * cos },
              { x: 中心x + 半宽 * cos + 半高 * sin, y: 中心y + 半宽 * sin - 半高 * cos },
              { x: 中心x + 半宽 * cos - 半高 * sin, y: 中心y + 半宽 * sin + 半高 * cos },
              { x: 中心x - 半宽 * cos - 半高 * sin, y: 中心y - 半宽 * sin + 半高 * cos },
            ];
      ...
      if (形状.形状 === "圆") {
        ...
        if (形状.旋转弧度 !== undefined) {
          if (水平翻转 && 垂直翻转) {
            ...
            // 垂直翻转：旋转角度取反（类似圆），但文本顺序保持不变（因为缩放比例.x不变）
            形状.旋转弧度 = -形状.旋转弧度;
          }
          // 标准化旋转弧度到 0-2π 范围
          while (形状.旋转弧度 > Math.PI * 2) {
  ...
  垂直翻转形状(形状) {
    ...
    if (形状.形状 === "圆") {
      ...
      // 文本：垂直翻转使用缩放比例，同时调整旋转角度
      // 垂直翻转时，旋转角度取反（类似圆），但文本顺序保持不变（因为缩放比例.x不变）
      if (!形状.缩放比例) {
        形状.缩放比例 = { x: 1, y: 1 };
      }
      形状.缩放比例.y = -形状.缩放比例.y;
  ...
  恢复形状状态(形状, 状态) {
    ...
    // 对于文本，如果已恢复顶点坐标组，则跳过更新文本形状（避免重新计算顶点覆盖已恢复的值）
    // 否则，更新文本形状以重新计算顶点坐标组
    if (形状.形状 === "文本") {
      if (状态.顶点坐标组 && 状态.顶点坐标组.length > 0) {
        // 已恢复顶点坐标组，只更新极值坐标和中心，不重新计算顶点
        // 顶点坐标组已在上面恢复（如果状态中有的话）
        // 但文本不在上面的条件中，需要手动恢复顶点坐标组
        形状.顶点坐标组 = 状态.顶点坐标组.map((坐标) => ({ x: 坐标.x, y: 坐标.y }));
        // 只更新极值坐标和中心，不调用更新文本形状
      } else {
        // 没有保存顶点坐标组，调用更新文本形状重新计算
        this.更新文本形状(形状);
      }
    }
  ...
  // 将多选形状组转换为"单位"数组（编组视为一个单位，非编组形状也视为一个单位）
  将多选形状组转换为单位数组() {
    const 单位数组 = [];
    const 已处理的组ID集合 = new Set();
    const 已处理的形状集合 = new Set();

    // 遍历多选形状组
    for (const 形状 of this.全局属性.多选形状组) {
      // 如果已经处理过，跳过
      if (已处理的形状集合.has(形状)) continue;

      if (this.形状已编组(形状)) {
        // 如果是编组内的形状，将整个编组作为一个单位
        const 组ID = 形状.所属组ID;
        if (!已处理的组ID集合.has(组ID)) {
          已处理的组ID集合.add(组ID);
          const 组内形状 = this.获取组内所有形状(组ID);
  ...
  移动形状到新位置(形状, 目标中心X, 目标中心Y) {
    ...
    // 根据形状类型移动
    if (形状.形状 === "文本") {
      // 文本：如果有旋转，直接移动顶点坐标组（保持旋转状态）；否则移动坐标
      if (形状.旋转弧度 && 形状.顶点坐标组 && 形状.顶点坐标组.length > 0) {
        // 旋转文本：直接移动所有顶点（类似矩形/箭头）
        形状.顶点坐标组.forEach((顶点) => {
          顶点.x += 偏移量X;
          顶点.y += 偏移量Y;
        });
  ...
  执行水平居中对齐() {
    ...
    单位数组.forEach((单位) => {
      if (单位.类型 === "编组") {
        // 编组：移动整个编组
        const 组内形状 = this.获取组内所有形状(单位.组ID);
        if (组内形状 && 组内形状.length >= 2) {
          // 计算编组的当前中心点
          const 原多选形状组 = this.全局属性.多选形状组;
          this.全局属性.多选形状组 = [...组内形状];
          ...
          if (组边界) {
            const 当前中心X = 组边界.centerX;
            const 偏移量X = 整体中心X - 当前中心X;
            ...
            组内形状.forEach((形状) => {
              ...
              this.移动形状到新位置(形状, 极值.中心.x + 偏移量X, 极值.中心.y);
            });
          }
        }
      } else {
        ...
        let 极值 = 单位.形状.极值坐标;
        if (!极值) {
          极值 = this.获取极值坐标(单位.形状);
        }
        const 形状中心X = 极值.中心.x;
        const 形状中心Y = 极值.中心.y;

        // 移动形状，使其中心点的X坐标等于整体中心X
        this.移动形状到新位置(单位.形状, 整体中心X, 形状中心Y);
      }
    });
    ...
    单位数组.forEach((单位) => {
      if (单位.类型 === "编组") {
        ...
        if (组内形状 && 组内形状.length >= 2) {
          ...
          if (组边界) {
            const 当前中心Y = 组边界.centerY;
            const 偏移量Y = 整体中心Y - 当前中心Y;
            ...
            组内形状.forEach((形状) => {
              ...
              this.移动形状到新位置(形状, 极值.中心.x, 极值.中心.y + 偏移量Y);
            });
          }
        }
      } else {
        ...
        this.移动形状到新位置(单位.形状, 形状中心X, 整体中心Y);
      }
    });
  ...
  执行水平均匀分布() {
    ...
    // 将多选形状组转换为"单位"数组（编组视为一个单位）
    const 单位数组 = this.将多选形状组转换为单位数组();
    if (单位数组.length < 3) return;

    // 按左边界排序
    单位数组.sort((a, b) => a.左边界 - b.左边界);

    // 第一个和最后一个单位保持不动
    const 第一个单位右边界 = 单位数组[0].右边界;
    const 最后一个单位左边界 = 单位数组[单位数组.length - 1].左边界;

    // 计算需要分布的中间单位数量（不包括第一个和最后一个）
    const 中间单位数量 = 单位数组.length - 2;
    if (中间单位数量 > 0) {
      // 计算总的可用空间（最后一个单位的左边界 - 第一个单位的右边界）
      const 总可用空间 = 最后一个单位左边界 - 第一个单位右边界;

      // 计算所有中间单位的总宽度（不包括第一个和最后一个）
      let 中间单位总宽度 = 0;
      for (let i = 1; i < 单位数组.length - 1; i++) {
        中间单位总宽度 += 单位数组[i].宽度;
      }

      // 计算均匀分布的间距
      const 均匀间距 = (总可用空间 - 中间单位总宽度) / (中间单位数量 + 1);

      // 分布中间的单位
      let 当前累积位置 = 第一个单位右边界 + 均匀间距;
      for (let i = 1; i < 单位数组.length - 1; i++) {
        const 当前单位宽度 = 单位数组[i].宽度;
        const 目标左边界 = 当前累积位置;
        const 目标中心X = 目标左边界 + 当前单位宽度 / 2;
        const 当前中心Y = 单位数组[i].中心Y;

        // 移动单位到新位置
        if (单位数组[i].类型 === "编组") {
          // 编组：移动整个编组
          const 组内形状 = 单位数组[i].组内形状;
          const 当前中心X = 单位数组[i].中心X;
          const 偏移量X = 目标中心X - 当前中心X;
          ...
          // 单个形状：正常移动
          this.移动形状到新位置(单位数组[i].形状, 目标中心X, 当前中心Y);
        }

        // 更新累积位置（当前位置 + 当前单位宽度 + 间距）
        当前累积位置 = 目标左边界 + 当前单位宽度 + 均匀间距;
      }
    }
  ...
  执行垂直均匀分布() {
    ...
    // 按上边界排序
    单位数组.sort((a, b) => a.上边界 - b.上边界);

    // 第一个和最后一个单位保持不动
    const 第一个单位下边界 = 单位数组[0].下边界;
    const 最后一个单位上边界 = 单位数组[单位数组.length - 1].上边界;

    // 计算需要分布的中间单位数量（不包括第一个和最后一个）
    const 中间单位数量 = 单位数组.length - 2;
    if (中间单位数量 > 0) {
      // 计算总的可用空间（最后一个单位的上边界 - 第一个单位的下边界）
      const 总可用空间 = 最后一个单位上边界 - 第一个单位下边界;

      // 计算所有中间单位的总高度（不包括第一个和最后一个）
      let 中间单位总高度 = 0;
      for (let i = 1; i < 单位数组.length - 1; i++) {
        中间单位总高度 += 单位数组[i].高度;
      }
      ...
      // 计算均匀分布的间距
      const 均匀间距 = (总可用空间 - 中间单位总高度) / (中间单位数量 + 1);

      // 分布中间的单位
      let 当前累积位置 = 第一个单位下边界 + 均匀间距;
      for (let i = 1; i < 单位数组.length - 1; i++) {
        const 当前单位高度 = 单位数组[i].高度;
        const 目标上边界 = 当前累积位置;
        const 目标中心Y = 目标上边界 + 当前单位高度 / 2;
        const 当前中心X = 单位数组[i].中心X;
        ...
        if (单位数组[i].类型 === "编组") {
          ...
          // 单个形状：正常移动
          this.移动形状到新位置(单位数组[i].形状, 当前中心X, 目标中心Y);
        }

        // 更新累积位置（当前位置 + 当前单位高度 + 间距）
        当前累积位置 = 目标上边界 + 当前单位高度 + 均匀间距;
      }
    }
  ...
  撤销() {
    ...
    if (最后操作.操作类型 === "添加基础形状") {
      ...
      if (最后操作.坐标) {
        ...
        // 对于文本，如果有顶点坐标组，应该从顶点坐标组反推坐标（与多选旋转一致）
        if (最后操作.形状.形状 === "文本") {
          // 恢复旋转弧度（如果有）
          if (最后操作.旋转弧度 !== undefined) {
            最后操作.形状.旋转弧度 = 最后操作.旋转弧度;
          }
          ...
          let 实际宽度;
          if (最后操作.形状.旋转弧度) {
            const 旋转弧度 = 最后操作.形状.旋转弧度;
            const cos = Math.cos(-旋转弧度);
            const sin = Math.sin(-旋转弧度);
            const 局部顶点 = 最后操作.形状.顶点坐标组.map((v) => {
              const dx = v.x - 中心x;
              const dy = v.y - 中心y;
              return {
                局部x: dx * cos - dy * sin,
            ...
            实际宽度 =
              Math.max(...最后操作.形状.顶点坐标组.map((v) => v.x)) -
              Math.min(...最后操作.形状.顶点坐标组.map((v) => v.x));
          }
          ...
          if (最后操作.形状.旋转弧度) {
            ...
            const cos = Math.cos(旋转弧度);
            const sin = Math.sin(旋转弧度);
            最后操作.形状.坐标.x = 中心x - 半宽 * cos;
            最后操作.形状.坐标.y = 中心y - 半宽 * sin;
          } else {
            最后操作.形状.坐标.x = 中心x - 半宽;
            最后操作.形状.坐标.y = 中心y;
          }
        ...
        旋转操作数组.forEach((旋转操作) => {
          ...
          if (形状.形状 === "圆" || 形状.形状 === "多边形" || 形状.形状 === "多角星") {
            ...
            if (旋转操作.顶点坐标组 && 旋转操作.顶点坐标组.length > 0) {
              ...
              // 如果恢复了顶点坐标组，只更新极值坐标，不调用更新文本形状（避免重新计算顶点坐标组）
              形状.极值坐标 = this.获取极值坐标(形状);
              形状.中心 = 形状.极值坐标.中心;
            } else {
              // 如果没有保存顶点坐标组，调用更新文本形状重新计算
              this.更新文本形状(形状);
              形状.极值坐标 = this.获取极值坐标(形状);
              形状.中心 = 形状.极值坐标.中心;
            }
      ...
      if (翻转操作数组 && 翻转操作数组.length > 0) {
        翻转操作数组.forEach((翻转操作) => {
          ...
          if (形状.形状 === "圆") {
            ...
            // 如果恢复了顶点坐标组，只更新极值坐标，不调用更新文本形状（避免重新计算顶点坐标组）
            if (变换前数据.顶点坐标组 && 变换前数据.顶点坐标组.length > 0) {
              // 极值坐标会在后面统一更新
            } else {
              // 如果没有保存顶点坐标组，调用更新文本形状重新计算
              this.更新文本形状(形状);
    ...
    } else if (最后操作.操作类型 === "添加文本和背景矩形并编组") {
      // 撤销添加文本和背景矩形并编组操作：先撤销编组，再删除文本和矩形形状
      const 文本形状 = 最后操作.文本形状;
      const 矩形形状 = 最后操作.矩形形状;
      const 组ID = 最后操作.组ID;
      ...
      if (矩形形状) {
        ...
        // 如果矩形形状是选中形状，清除选中状态
        if (矩形形状 === this.全局属性.选中形状) {
          this.全局属性.选中形状 = null;
        }

        // 从基础形状对象组中移除矩形形状
        const 矩形形状索引 = this.数据集.基础形状对象组.indexOf(矩形形状);
        if (矩形形状索引 !== -1) {
          this.数据集.基础形状对象组.splice(矩形形状索引, 1);
        }
      }
  ...
  绘制基础形状对象组() {
    // 在文本编辑模式下，即使没有形状也要绘制操作说明
    if (this.全局标志.辅助视觉效果 && this.全局标志.文本编辑中) {
      ...
      const 段间距 = 5;
      for (const 操作说明 of this.操作说明.文本) {
        if (this.操作说明.文本.indexOf(操作说明) === 0) continue;
    ...
    for (const 形状对象 of this.数据集.基础形状对象组) {
      ...
      if (形状对象.形状 === "文本") {
        this.绘制文本(形状对象);
        // 缩放时绘制文本使用的坐标点（调试用）
        /* if ((this.全局标志.缩放中 || this.全局标志.旋转中) && 形状对象.已选中 && 形状对象.坐标) {
          this.ctx.save();
          ...
          // 绘制文本坐标点（左边中心点）
          this.ctx.beginPath();
          this.ctx.arc(形状对象.坐标.x, 形状对象.坐标.y, 5, 0, Math.PI * 2);
          this.ctx.fill();
          this.ctx.stroke();

          // 可选：绘制文本中心点（用于对比）
          // const 中心x = 形状对象.坐标.x + 形状对象.尺寸.宽 / 2;
          // const 中心y = 形状对象.坐标.y;
          let 中心x, 中心y;
          if (形状对象.旋转弧度) {
            const 半宽 = 形状对象.尺寸.宽 / 2;
            const cos = Math.cos(形状对象.旋转弧度);
            const sin = Math.sin(形状对象.旋转弧度);
            中心x = 形状对象.坐标.x + 半宽 * cos;
            中心y = 形状对象.坐标.y + 半宽 * sin;
          } else {
            中心x = 形状对象.坐标.x + 形状对象.尺寸.宽 / 2;
            中心y = 形状对象.坐标.y;
          }
          this.ctx.fillStyle = "green";
          this.ctx.strokeStyle = "green";
          this.ctx.beginPath();
          this.ctx.arc(中心x, 中心y, 4, 0, Math.PI * 2);
          this.ctx.fill();
          this.ctx.stroke();
          this.ctx.restore();
        } */
    ...
    if (this.全局属性.多选形状组.length > 0) {
      ...
      if (边界) {
        ...
        // 只有当多选包含编组，但不是整个多选就是一个编组时，才需要扩展边界
        if (!整个多选就是一个编组) {
          // 检查多选形状组中是否有编组，如果有，根据编组位置调整多选整体边界
          const 外边距 = 25; // 与绘制多选整体交互框中的外边距一致
          const 容差 = 5; // 判断编组是否位于边缘的容差
          const 扩展距离 = 25; // 需要扩展的距离（与编组交互框的外边距一致）

          // 计算多选整体交互框的边界（包括外边距）
          const 整体左边界 = 边界.minX - 外边距;
          const 整体右边界 = 边界.minX + 边界.width + 外边距;
          const 整体上边界 = 边界.minY - 外边距;
          const 整体下边界 = 边界.minY + 边界.height + 外边距;
            ...
            if (this.形状已编组(形状)) {
              ...
              if (!已处理的组ID集合.has(组ID)) {
                ...
                if (组内形状 && 组内形状.length >= 2) {
                  ...
                  // 计算组内所有形状的边界
                  const 组边界 = this.获取多选形状的边界();
                  if (组边界 && 组边界.width > 0 && 组边界.height > 0) {
                    // 计算编组交互框的边界（包括外边距）
                    const 组左边界 = 组边界.minX - 外边距;
                    const 组右边界 = 组边界.minX + 组边界.width + 外边距;
                    const 组上边界 = 组边界.minY - 外边距;
                    const 组下边界 = 组边界.minY + 组边界.height + 外边距;

                    // 判断编组是否位于多选整体的边缘
                    if (Math.abs(组左边界 - 整体左边界) <= 容差) {
                      需要扩展左 = true;
                    }
                    if (Math.abs(组右边界 - 整体右边界) <= 容差) {
                      需要扩展右 = true;
                    }
                    if (Math.abs(组上边界 - 整体上边界) <= 容差) {
                      需要扩展上 = true;
                    }
                    if (Math.abs(组下边界 - 整体下边界) <= 容差) {
                      需要扩展下 = true;
                    }
                  }
          ...
          // 重新计算中心位置（扩展边界后，中心位置会改变）
          边界.centerX = 边界.minX + 边界.width / 2;
          边界.centerY = 边界.minY + 边界.height / 2;
        }
      ...
      // 检查多选形状组中是否有编组，如果有，为每个编组绘制预览交互框
      const 已处理的组ID集合 = new Set();
      for (const 形状 of this.全局属性.多选形状组) {
        if (this.形状已编组(形状)) {
          const 组ID = 形状.所属组ID;
          // 如果这个编组还没有处理过，且组内至少有2个形状
          if (!已处理的组ID集合.has(组ID)) {
            已处理的组ID集合.add(组ID);
            const 组内形状 = this.获取组内所有形状(组ID);
            if (组内形状 && 组内形状.length >= 2) {
              ...
              const 组边界 = this.获取多选形状的边界();
              if (组边界 && 组边界.width > 0 && 组边界.height > 0) {
                // 判断特殊情况：如果Shift键按下，且多选中有不在该编组内的形状，且鼠标在编组内，使用虚线
                let 虚线模式 = null; // 默认为null，使用默认逻辑（实线）
                if (this.全局标志.Shift已按下) {
                  // 检查多选形状组中是否有不在该编组内的形状
                  const 有不在该编组内的形状 = 原多选形状组.some((形状) => !形状.所属组ID || 形状.所属组ID !== 组ID);
                  // 只有当鼠标在编组内时，才使用虚线
                  if (有不在该编组内的形状 && this.鼠标位于编组内(组边界)) {
                    虚线模式 = [10, 5];
                  }
                }

                // 绘制编组的预览交互框（不透明度35%，强制已编组为true，自定义虚线模式）
                this.绘制多选整体交互框(组边界, 0.35, true, 虚线模式);
              }
    ...
    // 绘制框选预览中的编组预览交互框
    if (this.全局标志.正在框选 && this.全局属性.框选预览编组ID集合.size > 0) {
      for (const 组ID of this.全局属性.框选预览编组ID集合) {
        const 组内形状 = this.获取组内所有形状(组ID);
        if (组内形状 && 组内形状.length >= 2) {
          // 临时将组内形状设置为多选形状组，以便计算边界和绘制交互框
          const 原多选形状组 = this.全局属性.多选形状组;
          ...
          const 组边界 = this.获取多选形状的边界();
          if (组边界 && 组边界.width > 0 && 组边界.height > 0) {
            // 绘制编组的预览交互框（不透明度35%，强制已编组为true）
            this.绘制多选整体交互框(组边界, 0.35, true, null);
          }
    ...
    if (this.全局标志.辅助视觉效果) {
      ...
      // 绘制移动操作说明（当正在拖拽任意形状、多选或编组时）
      if (
        this.全局标志.辅助视觉效果 &&
        (this.全局标志.拖拽中 || this.全局标志.多选拖拽中 || this.全局属性.拖拽编组ID !== null)
      ) {
        const 特殊名词组 = ["↑", "↓", "←", "→", "[", "]", "Shift", "Enter", "ESC", "空格", "Ctrl"];
        const 上距离 = 20;
        const 右距离 = 20;
        const 起始坐标 = {
          x: this.canvas.offsetWidth - 右距离,
    ...
    // 如果按住Ctrl键且正在拖拽，绘制移动轨道
    if (this.键盘状态.Control && this.全局属性.点击坐标 && this.全局属性.鼠标坐标) {
      if (this.全局标志.拖拽中 || this.全局标志.多选拖拽中) {
        this.绘制移动轨道();
      }
    }
  }
  ...
  计算交互框尺寸(形状对象) {
    if (!形状对象 || !形状对象.极值坐标) return null;

    const 外边距 = 10 + 形状对象.描边宽度 * (形状对象.形状 === "多角星" ? 1 : 0.5);

    // 对于所有形状，都使用旋转后的极值坐标（AABB）来计算交互框尺寸
    // 这样旋转后的形状的轨道尺寸会正确反映旋转后的交互框尺寸
    const 尺寸 = {
      width: 形状对象.极值坐标.右 - 形状对象.极值坐标.左 + 外边距 * 2,
      height: 形状对象.极值坐标.下 - 形状对象.极值坐标.上 + 外边距 * 2,
    };

    // 如果尺寸无效，返回null
    if (尺寸.width <= 0 || 尺寸.height <= 0) {
      return null;
    }
  ...
  计算多选交互框尺寸() {
    ...
    // 计算扩展后的边界（与绘制多选整体交互框的逻辑一致）
    边界 = this.计算多选扩展边界(边界);

    const 外边距 = 25; // 与绘制多选整体交互框中的外边距一致
    return {
      width: 边界.width + 外边距 * 2,
      height: 边界.height + 外边距 * 2,
    };
  }
  ...
  绘制移动轨道() {
    if (!this.全局标志.辅助视觉效果) return;
    if (!this.键盘状态.Control) return;
    if (!this.全局属性.点击坐标 || !this.全局属性.鼠标坐标) return;
    if (!this.全局属性.初始交互框尺寸 || !this.全局属性.初始交互框位置) return;

    this.ctx.save();
    this.ctx.setLineDash([]);

    // 获取画布尺寸
    const 画布宽度 = this.canvas.width / this.dpr;
    const 画布高度 = this.canvas.height / this.dpr;

    // 如果正在单独拖拽一个形状（Shift+拖拽单独移动），使用该形状的交互框尺寸
    let 轨道高度 = this.全局属性.初始交互框尺寸.height;
    let 轨道宽度 = this.全局属性.初始交互框尺寸.width;
    let 水平轨道Y = this.全局属性.初始交互框位置.y;
    let 垂直轨道X = this.全局属性.初始交互框位置.x;

    if (this.全局标志.Shift单独拖拽中 && this.全局属性.Shift点击待移除形状) {
      // 单独拖拽形状时，使用该形状的交互框尺寸和初始位置
      const 单独移动形状 = this.全局属性.Shift点击待移除形状;
      const 初始位置 = this.全局属性.多选初始位置组.find((位置) => 位置.形状 === 单独移动形状);

      if (初始位置 && 初始位置.按下时坐标) {
        // 创建一个临时形状对象，使用初始位置来计算初始交互框尺寸和位置
        const 临时形状 = { ...单独移动形状 };
        临时形状.坐标 = { ...初始位置.按下时坐标 };
        if (初始位置.按下时顶点坐标组) {
          临时形状.顶点坐标组 = 初始位置.按下时顶点坐标组.map((v) => ({ ...v }));
        }

        // 计算初始交互框尺寸
        const 形状交互框尺寸 = this.计算交互框尺寸(临时形状);
        if (形状交互框尺寸) {
          轨道高度 = 形状交互框尺寸.height;
          轨道宽度 = 形状交互框尺寸.width;

          // 计算初始极值坐标
          const 初始极值 = this.获取极值坐标(临时形状);
          if (初始极值) {
            水平轨道Y = 初始极值.中心.y;
            垂直轨道X = 初始极值.中心.x;
          }
    ...
    } else if (this.全局属性.拖拽编组ID !== null) {
      // 拖拽多选内的编组时，使用该编组的交互框尺寸和初始位置
      const 组内形状 = this.获取组内所有形状(this.全局属性.拖拽编组ID);
      if (组内形状 && 组内形状.length >= 2) {
        // 使用初始位置计算编组的初始边界
        const 临时形状组 = [];
        for (const 形状 of 组内形状) {
          const 初始位置 = this.全局属性.多选初始位置组.find((位置) => 位置.形状 === 形状);
          if (初始位置) {
            // 创建临时形状对象，使用初始位置
            const 临时形状 = { ...形状 };
            if (初始位置.按下时坐标) {
            ...
            if (初始位置.按下时旋转弧度 !== undefined) {
              临时形状.旋转弧度 = 初始位置.按下时旋转弧度;
            }
            // 重新计算极值坐标
            临时形状.极值坐标 = this.获取极值坐标(临时形状);
            临时形状组.push(临时形状);
          } else {
            // 如果没有初始位置，使用当前形状
            临时形状组.push(形状);
        ...
        // 临时将组内形状设置为多选形状组，以便计算边界
        const 原多选形状组 = this.全局属性.多选形状组;
        this.全局属性.多选形状组 = 临时形状组;

        // 计算组内所有形状的初始边界
        const 组边界 = this.获取多选形状的边界();
        if (组边界 && 组边界.width > 0 && 组边界.height > 0) {
          const 外边距 = 25; // 与绘制多选整体交互框中的外边距一致
          轨道高度 = 组边界.height + 外边距 * 2;
          轨道宽度 = 组边界.width + 外边距 * 2;
          水平轨道Y = 组边界.centerY;
          垂直轨道X = 组边界.centerX;
        }
      ...
      // 正常拖拽时，使用保存的初始交互框尺寸和位置
      轨道高度 = this.全局属性.初始交互框尺寸.height;
      轨道宽度 = this.全局属性.初始交互框尺寸.width;
      水平轨道Y = this.全局属性.初始交互框位置.y;
      垂直轨道X = this.全局属性.初始交互框位置.x;
    }
    ...
    this.ctx.fillStyle = "#ffffff03";
    this.ctx.lineWidth = 2;

    // 绘制水平轨道：从Canvas左边界到右边界，高度与交互框一致
    // 位置：初始拖拽时的垂直位置（交互框的中心Y）
    // 填充
    this.ctx.fillRect(0, 水平轨道Y - 轨道高度 / 2, 画布宽度, 轨道高度);
    // 描边（上下两条边）
    this.ctx.beginPath();
    this.ctx.moveTo(0, 水平轨道Y - 轨道高度 / 2);
    this.ctx.lineTo(画布宽度, 水平轨道Y - 轨道高度 / 2);
    this.ctx.moveTo(0, 水平轨道Y + 轨道高度 / 2);
    this.ctx.lineTo(画布宽度, 水平轨道Y + 轨道高度 / 2);
    this.ctx.stroke();

    // 绘制垂直轨道：从Canvas上边界到下边界，宽度与交互框一致
    // 位置：初始拖拽时的水平位置（交互框的中心X）
    // 填充
    this.ctx.fillRect(垂直轨道X - 轨道宽度 / 2, 0, 轨道宽度, 画布高度);
    // 描边（左右两条边）
    this.ctx.beginPath();
    this.ctx.moveTo(垂直轨道X - 轨道宽度 / 2, 0);
    this.ctx.lineTo(垂直轨道X - 轨道宽度 / 2, 画布高度);
    this.ctx.moveTo(垂直轨道X + 轨道宽度 / 2, 0);
    this.ctx.lineTo(垂直轨道X + 轨道宽度 / 2, 画布高度);
    this.ctx.stroke();
  ...
  绘制缩放旋转操作说明() {
    // 检查是否正在旋转或缩放（单个形状或多选）
    const 正在操作单个 = this.全局标志.旋转中 || this.全局标志.缩放中;
    const 正在操作多选 = this.全局标志.多选旋转中 || this.全局标志.多选缩放中;
    const 正在操作 = 正在操作单个 || 正在操作多选;
    ...
    if (this.交互框) {
      ...
      if (!正在操作) {
        ...
        if (鼠标位于边界状态) {
          鼠标位于边界 = 鼠标位于边界状态.上 || 鼠标位于边界状态.下 || 鼠标位于边界状态.左 || 鼠标位于边界状态.右;
        }
      }
    } else if (this.全局属性.多选形状组.length > 0) {
      // 多选或编组的情况
      if (!正在操作) {
        ...
        if (多选边界位置) {
          鼠标位于边界 =
            多选边界位置.上 || 多选边界位置.下 || 多选边界位置.左 || 多选边界位置.右 || 多选边界位置.位于句柄;
        }
      }
    }

    // 如果既没有正在操作，也没有鼠标位于边界，则不显示说明
    if (!正在操作 && !鼠标位于边界) return;
