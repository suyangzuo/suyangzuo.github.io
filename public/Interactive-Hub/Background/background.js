...
拖拽区.addEventListener("dragstart", (event) => {
  const 操作区边界矩形 = 操作区.getBoundingClientRect();
  const 鼠标位置_水平 = event.clientX;
  const 鼠标位置_垂直 = event.clientY;
  拖拽偏移.水平 = 操作区边界矩形.left - 鼠标位置_水平;
  拖拽偏移.垂直 = 操作区边界矩形.top - 鼠标位置_垂直;
  正在拖拽操作区 = true;
  操作区.style.transition = "none";
  event.dataTransfer.setData("text/plain", "背景-操作区");
...
背景总区.addEventListener("dragover", (event) => {
  ...
  更新操作区拖拽位置(event.clientX, event.clientY);
});
...
拖拽区.addEventListener("dragend", () => {
  结束操作区拖拽();
});

function 更新操作区拖拽位置(鼠标位置_水平, 鼠标位置_垂直) {
  if (鼠标位置_水平 === 0 && 鼠标位置_垂直 === 0) {
    return;
  }

  const 背景总区边界矩形 = 背景总区.getBoundingClientRect();
  const 操作区边界矩形 = 操作区.getBoundingClientRect();
  const 目标left = 鼠标位置_水平 - 背景总区边界矩形.left + 拖拽偏移.水平;
  const 目标top = 鼠标位置_垂直 - 背景总区边界矩形.top + 拖拽偏移.垂直;
  const 最大left = Math.max(背景总区边界矩形.width - 操作区边界矩形.width, 0);
  const 最大top = Math.max(背景总区边界矩形.height - 操作区边界矩形.height, 0);
  const 实际left = Math.min(Math.max(目标left, 0), 最大left);
  const 实际top = Math.min(Math.max(目标top, 0), 最大top);

  操作区.style.left = `${实际left}px`;
  操作区.style.top = `${实际top}px`;
}
