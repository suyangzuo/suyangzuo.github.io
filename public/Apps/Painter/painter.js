class 随心绘 {
  ...
  添加颜色拾取器事件() {
    ...
    this.描边颜色拾取器.on("change", (color) => {
      ...
      const 处于文本状态 = 
        ...
        (this.全局属性.选中形状 && this.全局属性.选中形状.形状 === "文本"); // 选中文本形状
  ...
  退出文本编辑模式() {
    ...
    // 如果文本为空且正在编辑已有文本，恢复原始文本内容
    if (!文本内容 && this.全局属性.当前编辑文本形状) {
      if (this.全局属性.当前编辑文本形状._原始文本内容) {
        this.全局属性.当前编辑文本形状.文本内容 = this.全局属性.当前编辑文本形状._原始文本内容;
  ...
  应用缩放(形状对象, 水平缩放比例, 垂直缩放比例, 缩放锚点) {
    if (形状对象.形状 === "文本") {
      // 文本形状：等比缩放，通过修改字号实现
      // 计算等比缩放比例（取两个方向的平均值）
      const 缩放比例 = (Math.abs(水平缩放比例) + Math.abs(垂直缩放比例)) / 2;
      
      // 更新字号
      形状对象.字号 = Math.round(形状对象.点击时字号 * 缩放比例);
      ...
      if (形状对象.点击时顶点坐标组 && 形状对象.顶点坐标组) {
        if (形状对象.旋转弧度) {
          ...
          for (let i = 0; i < 形状对象.顶点坐标组.length; i++) {
            ...
            // 在局部坐标系中应用等比缩放
            const 缩放后局部x = 局部x * 缩放比例;
            const 缩放后局部y = 局部y * 缩放比例;
          ...
          for (let i = 0; i < 形状对象.顶点坐标组.length; i++) {
            形状对象.顶点坐标组[i].x = 缩放锚点.x + (形状对象.点击时顶点坐标组[i].x - 缩放锚点.x) * 缩放比例;
            形状对象.顶点坐标组[i].y = 缩放锚点.y + (形状对象.点击时顶点坐标组[i].y - 缩放锚点.y) * 缩放比例;
          }
        }
      ...
      // 计算几何中心
      const 中心x = (形状对象.顶点坐标组[0].x + 
                   形状对象.顶点坐标组[1].x + 
                   形状对象.顶点坐标组[2].x + 
                   形状对象.顶点坐标组[3].x) / 4;
      const 中心y = (形状对象.顶点坐标组[0].y + 
                   形状对象.顶点坐标组[1].y + 
                   形状对象.顶点坐标组[2].y + 
                   形状对象.顶点坐标组[3].y) / 4;
