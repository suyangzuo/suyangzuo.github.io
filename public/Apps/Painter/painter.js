        ...
        if (this.交互框) {
          ...
          if (this.全局标志.拖拽中) {
            ...
            if (this.全局标志.缩放中) {
              ...
              // 箭头特殊处理：按住Shift键时，旋转角度量化到45度（π/4）的倍数
              if (this.全局属性.选中形状.形状 === "箭头" && this.键盘状态.Shift) {
                // 计算目标旋转弧度
  ...
  应用多选形状缩放(形状, 初始位置, 水平缩放比例, 垂直缩放比例, 缩放锚点, 初始边界) {
    ...
    if (形状.形状 === "文本") {
      ...
      if (初始位置.按下时顶点坐标组 && 初始位置.按下时顶点坐标组.length >= 4) {
        // 计算文本的初始中心（几何中心）
        const 文本初始中心 = {
          x: (初始位置.按下时顶点坐标组[0].x + 初始位置.按下时顶点坐标组[1].x + 初始位置.按下时顶点坐标组[2].x + 初始位置.按下时顶点坐标组[3].x) / 4,
          y: (初始位置.按下时顶点坐标组[0].y + 初始位置.按下时顶点坐标组[1].y + 初始位置.按下时顶点坐标组[2].y + 初始位置.按下时顶点坐标组[3].y) / 4,
        };
    ...
    if (最后操作.操作类型 === "添加基础形状") {
      ...
      if (组ID && 添加的形状 && 操作前组形状 && this.数据集.形状组[组ID]) {
        ...
        if (操作前选中形状 && !this.形状已编组(操作前选中形状) && this.数据集.基础形状对象组.includes(操作前选中形状)) {
          ...
          const 有效的未编组形状 = 操作前多选形状组.filter((形状) => 
            this.数据集.基础形状对象组.includes(形状) && !形状.所属组ID
          );
