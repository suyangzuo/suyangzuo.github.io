class CubicBezierEditor {
  ...
  drawPoint(x, y, color, label, isHovered) {
    ...
    this.ctx.fillStyle = "#ffffff";
    this.ctx.font = '12px "Google Sans Code", "Consolas", sans-serif';
  ...
  drawCoordinateLabels(cp1, cp2) {
    ...
    this.ctx.fillText(
      y1.toString(),
      cp1.x + 20 + this.ctx.measureText(x1.toString()).width + this.ctx.measureText(", ").width,
      cp1.y + 15
    );
    ...
    this.ctx.fillText(
      y2.toString(),
      cp2.x + 20 + this.ctx.measureText(x2.toString()).width + this.ctx.measureText(", ").width,
      cp2.y + 15
    );
  }
  ...
  checkPresetMatch() {
    const current = [this.controlPoints.x1, this.controlPoints.y1, this.controlPoints.x2, this.controlPoints.y2];

    for (const [presetName, presetValues] of Object.entries(this.presets)) {
      const isMatch = presetValues.every((value, index) => Math.abs(value - current[index]) < 0.01);
      if (isMatch) {
        return presetName;
      }
    }
