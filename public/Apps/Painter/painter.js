class 随心绘 {
  ...
  执行缩放(形状对象, 是否等比) {
    ...
    // 对于旋转后的圆形、多边形、多角星，需要在局部坐标系中计算缩放比例
    const 需要局部坐标系 =
      (形状对象.形状 === "圆" || 形状对象.形状 === "多边形" || 形状对象.形状 === "多角星") && 形状对象.旋转弧度;
  ...
  绘制交互框(形状对象) {
    ...
    this.ctx.strokeStyle = this.全局标志.旋转中 && !this.全局标志.辅助视觉效果 ? "transparent" : "greenyellow";
    this.ctx.stroke();
    this.ctx.lineDashOffset = 虚线长度;
    this.ctx.strokeStyle = this.全局标志.旋转中 && !this.全局标志.辅助视觉效果 ? "transparent" : "#234";
    this.ctx.stroke();

    const 句柄半径 = 8;
    this.ctx.strokeStyle = this.全局标志.旋转中 && !this.全局标志.辅助视觉效果 ? "transparent" : "#aaa";
    this.ctx.lineWidth = 4;
    this.ctx.fillStyle = this.全局标志.旋转中 && !this.全局标志.辅助视觉效果 ? "transparent" : "#333";
    ...
    const 角点坐标组 = [
      { x: this.交互框.坐标.x, y: this.交互框.坐标.y }, // 左上
      { x: this.交互框.坐标.x + this.交互框.尺寸.width, y: this.交互框.坐标.y }, // 右上
      { x: this.交互框.坐标.x + this.交互框.尺寸.width, y: this.交互框.坐标.y + this.交互框.尺寸.height }, // 右下
      { x: this.交互框.坐标.x, y: this.交互框.坐标.y + this.交互框.尺寸.height }, // 左下
    ];
