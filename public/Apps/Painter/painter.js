class 随心绘 {
  ...
  鼠标位于交互框内() {
    ...
    return 局部X >= -半宽 + 容差 && 局部X <= 半宽 - 容差 && 局部Y >= -半高 + 容差 && 局部Y <= 半高 - 容差;
  }
  ...
  绘制交互框(形状对象) {
    ...
    if (形状对象.形状 === "矩形" || 形状对象.形状 === "圆") {
      // 圆形也使用极值坐标，不再跟随旋转
      // 始终重新计算圆形的极值坐标（可能旋转或缩放后需要更新）
      if (形状对象.形状 === "圆") {
        形状对象.极值坐标 = this.获取极值坐标(形状对象);
      }
    ...
    // 所有形状（包括圆）都使用相同的绘制方式，不再对圆形应用旋转变换
