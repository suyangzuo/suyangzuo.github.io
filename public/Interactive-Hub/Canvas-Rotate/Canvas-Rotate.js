class Canvas旋转演示器 {
  ...
  生成重置目标形状数组() {
    ...
    const 目标形状数组 = this.默认形状数组.map(形状 => {
      const 副本 = { ...形状 };
      if (形状.中心点) {
        副本.中心点 = { ...形状.中心点 };
      }
  ...
  插值形状数组(起始形状数组, 目标形状数组, t) {
    const lerp = (a, b) => a + (b - a) * t;
    return 目标形状数组.map((目标形状, 索引) => {
      const 起始形状 = 起始形状数组[索引] || 目标形状;
      const 插值后 = { ...目标形状 };

      if (起始形状.中心点 && 目标形状.中心点) {
        插值后.中心点 = {
          x: lerp(起始形状.中心点.x, 目标形状.中心点.x),
          y: lerp(起始形状.中心点.y, 目标形状.中心点.y),
        };
      } else if (目标形状.中心点) {
        插值后.中心点 = { ...目标形状.中心点 };
      }

      if (Array.isArray(起始形状.顶点) && Array.isArray(目标形状.顶点)) {
        插值后.顶点 = 目标形状.顶点.map((目标顶点, 顶点索引) => {
          const 起始顶点 = 起始形状.顶点[顶点索引] || 目标顶点;
          return {
            x: lerp(起始顶点.x, 目标顶点.x),
            y: lerp(起始顶点.y, 目标顶点.y),
          };
        });
      } else if (Array.isArray(目标形状.顶点)) {
        插值后.顶点 = 目标形状.顶点.map(v => ({ ...v }));
      }
  ...
  重置() {
    ...
    const 起始原点 = { x: this.原点.x, y: this.原点.y };
    const 起始旋转弧度 = this.旋转弧度;
    const 起始形状数组 = this.形状数组.map(形状 => {
      const 副本 = { ...形状 };
      if (形状.中心点) {
        副本.中心点 = { ...形状.中心点 };
      }
    ...
    const 执行动画 = (当前时间) => {
      ...
      this.原点.x = 起始原点.x + (目标原点.x - 起始原点.x) * 进度;
      this.原点.y = 起始原点.y + (目标原点.y - 起始原点.y) * 进度;
      this.旋转弧度 = 起始旋转弧度 + (目标旋转弧度 - 起始旋转弧度) * 进度;
      this.初始旋转弧度 = this.旋转弧度;
      this.形状数组 = this.插值形状数组(起始形状数组, 目标形状数组, 进度);
