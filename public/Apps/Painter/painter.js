class 随心绘 {
  ...
  鼠标位于交互框内() {
    ...
    // 获取旋转中心点
    // 对于多边形和多角星，旋转中心是形状的中心，不是交互框的几何中心
    let 中心X, 中心Y;
    const 形状对象 = this.交互框.交互形状;
    if (形状对象 && (形状对象.形状 === "多边形" || 形状对象.形状 === "多角星")) {
      中心X = 形状对象.坐标.x;
      中心Y = 形状对象.坐标.y;
    } else {
      // 对于其他形状，使用交互框的几何中心
    ...
    return 局部X >= 局部左边界 + 容差 && 局部X <= 局部右边界 - 容差 && 局部Y >= 局部上边界 + 容差 && 局部Y <= 局部下边界 - 容差;
  }
  ...
  添加canvas鼠标抬起事件() {
    this.canvas.addEventListener("mouseup", () => {
      // 在重置缩放标志之前，检查是否需要规范化矩形顶点顺序
      if (this.全局标志.缩放中 && this.全局属性.选中形状 && this.全局属性.选中形状.形状 === "矩形") {
  ...
  执行缩放(形状对象, 是否等比) {
    ...
    // 对于旋转后的圆形、多边形、多角星、矩形，需要在局部坐标系中计算缩放比例
    const 需要局部坐标系 =
      (形状对象.形状 === "圆" || 形状对象.形状 === "多边形" || 形状对象.形状 === "多角星" || 形状对象.形状 === "矩形") && 形状对象.旋转弧度;
  ...
  应用缩放(形状对象, 水平缩放比例, 垂直缩放比例, 缩放锚点) {
    if (形状对象.形状 === "圆") {
      ...
      形状对象.极值坐标 = this.获取极值坐标(形状对象);
    } else if (形状对象.形状 === "矩形" || 形状对象.形状 === "直线" || 形状对象.形状 === "自由") {
      // 对于顶点坐标组，应用缩放比例（包括负值），实现镜像翻转
      if (形状对象.点击时顶点坐标组) {
        // 检查是否有旋转（仅矩形需要特殊处理）
        if (形状对象.形状 === "矩形" && 形状对象.旋转弧度) {
          // 对于旋转后的矩形，在局部坐标系中缩放顶点
          const 旋转弧度 = 形状对象.旋转弧度;
          const cos = Math.cos(-旋转弧度);
          const sin = Math.sin(-旋转弧度);
  ...
  规范化矩形顶点顺序(形状对象) {
    ...
    // 先计算当前的旋转角度（基于现有顶点）
    const 当前旋转角度 = this.计算矩形旋转角度(形状对象);
    
    // 计算矩形中心点
    const 中心X = (顶点[0].x + 顶点[1].x + 顶点[2].x + 顶点[3].x) / 4;
    const 中心Y = (顶点[0].y + 顶点[1].y + 顶点[2].y + 顶点[3].y) / 4;
    
    // 在局部坐标系中重新排序（使用计算出的旋转角度）
    const cos = Math.cos(-当前旋转角度);
    const sin = Math.sin(-当前旋转角度);
  ...
  绘制交互框(形状对象, 透明度 = 1.0, 显示句柄 = true) {
    ...
    if (形状对象.形状 === "圆") {
      ...
      // 计算未旋转时的原始尺寸（通过顶点计算）
      // 假设矩形有4个顶点，计算第一条边的长度作为宽，第二条边的长度作为高
    ...
    if (旋转弧度) {
      ...
      // 根据形状类型确定旋转中心
      if (形状对象.形状 === "矩形" || 形状对象.形状 === "直线" || 形状对象.形状 === "自由") {
        // 对于基于顶点的形状，使用极值坐标的中心
        中心X = 形状对象.极值坐标.中心.x;
        中心Y = 形状对象.极值坐标.中心.y;
      } else {
    ...
    this.ctx.strokeStyle = 旋转时隐藏 ? "transparent" : `rgba(173, 255, 47, ${透明度})`;
    this.ctx.stroke();
    this.ctx.lineDashOffset = 虚线长度;
    this.ctx.strokeStyle = 旋转时隐藏 ? "transparent" : `rgba(34, 51, 68, ${透明度})`;
    this.ctx.stroke();
