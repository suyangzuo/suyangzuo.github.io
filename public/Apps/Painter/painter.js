class 随心绘 {
  ...
  绘制多选整体交互框(边界, 不透明度 = 1.0, 强制已编组 = false) {
    ...
    // 如果已编组，在整体交互框正上方绘制"G"文本
    if (已编组) {
      this.ctx.save();
      this.ctx.globalAlpha = 1;
      this.ctx.fillStyle = "#4a9eff";
      this.ctx.font = "bold 16px 'Noto Sans SC', '微软雅黑', sans-serif";
      this.ctx.textAlign = "center";
      this.ctx.textBaseline = "bottom";

      // 计算文本位置：在整体交互框的正上方（中心点上方）
      const 文本X = minX + width / 2;
      const 文本Y = minY - 外边距 - 5;
      this.ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
      this.ctx.fillStyle = "white";
      this.ctx.fillText("G", 文本X, 文本Y);
