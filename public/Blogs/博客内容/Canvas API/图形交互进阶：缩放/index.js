...
class 热区优先级 {
  ...
  初始化() {
    this.cvs = document.getElementById("cvs-热区优先级");
    this.ctx = this.cvs.getContext("2d");
    this.配置 = {
      画布: {
        背景色: "#0a0a0a",
      ...
      颜色: {
        本体填充: "#ffffff0c",
        本体高亮填充: "darkgoldenrod",
        边: "#9b9b9b",
        边高亮: "#ffd166",
        角: "#4a9eff",
        角高亮: "#ffd166",
        热区角描边: "#f97316",
        热区角填充: "rgba(249, 115, 22, 0.16)",
        热区边描边: "#38bdf8",
        热区边填充: "rgba(56, 189, 248, 0.05)",
        热区本体描边: "#34d399",
        热区本体填充: "#ffffff0c",
      },
      复选框: {
        ...
        尺寸: 18,
        圆角: 4,
        字体: "14px 'Noto Sans SC', 微软雅黑, sans-serif",
        标签间距: 8,
        内边距: 8,
        背景色: "#0b1220",
        背景悬停色: "#1a243a",
      ...
      排序区: {
        ...
        圆角: 10,
        标题: "热区优先级顺序",
        标题字体: "14px 'Noto Sans SC', 微软雅黑, sans-serif",
        标题色: "#e2e8f0",
        序号字体: "13px 'Google Sans Code', 'JetBrains Mono', Consolas, monospace",
        序号色: "#94a3b8",
        行起始Y: 50,
        行高: 44,
        序号X: 20,
        ...
        芯片高: 30,
        芯片圆角: 8,
        芯片字体: "14px 'Noto Sans SC', 微软雅黑, sans-serif",
      ...
      分组样式: {
        角: { 填充: "rgba(249, 115, 22, 0.2)", 描边: "#fb923c", 文字: "#ffedd5" },
        边: { 填充: "rgba(56, 189, 248, 0.22)", 描边: "#38bdf8", 文字: "#e0f2fe" },
        本体: { 填充: "rgba(52, 211, 153, 0.2)", 描边: "#34d399", 文字: "#d1fae5" },
      },
    };
    ...
    this.热区顺序 = ["角", "边", "本体"];
    this.芯片状态 = {
      角: { x: 0, y: 0, 目标x: 0, 目标y: 0 },
      边: { x: 0, y: 0, 目标x: 0, 目标y: 0 },
      本体: { x: 0, y: 0, 目标x: 0, 目标y: 0 },
    };
    this.拖拽芯片 = null;
    this.拖拽偏移 = { x: 0, y: 0 };
    this.边界矩形 = this.cvs.getBoundingClientRect();
  ...
  安装事件() {
    this._onMouseDown = this.鼠标按下.bind(this);
    this._onMouseMove = this.鼠标移动.bind(this);
    this._onMouseUp = this.鼠标抬起.bind(this);
    this._onMouseLeave = this.鼠标离开.bind(this);
    this._onResize = this._resize.bind(this);

    this.cvs.addEventListener("mousedown", this._onMouseDown);
    window.addEventListener("mousemove", this._onMouseMove);
    window.addEventListener("mouseup", this._onMouseUp);
    this.cvs.addEventListener("mouseleave", this._onMouseLeave);
  ...
  _命中角热区(p) {
    const 角点 = this._获取角点();
    const 半径 = this.配置.热区.角命中半径;
    for (let i = 0; i < 4; i++) {
      const d = Math.hypot(p.x - 角点[i].x, p.y - 角点[i].y);
      if (d <= 半径) return i;
    }
    return -1;
  }
  ...
  _命中边热区(p) {
    const r = this.矩形;
    const t = this.配置.热区.边命中厚度;
    const half = t / 2;

    if (p.x >= r.x && p.x <= r.x + r.w && p.y >= r.y - half && p.y <= r.y + half) return 0;
    if (p.y >= r.y && p.y <= r.y + r.h && p.x >= r.x + r.w - half && p.x <= r.x + r.w + half) return 1;
    if (p.x >= r.x && p.x <= r.x + r.w && p.y >= r.y + r.h - half && p.y <= r.y + r.h + half) return 2;
    if (p.y >= r.y && p.y <= r.y + r.h && p.x >= r.x - half && p.x <= r.x + half) return 3;
    return -1;
  }

  _命中本体热区(p) {
    return this._点在矩形内(p, this.矩形);
  }

  _命中复选框(p) {
    const cfg = this.配置.复选框;
    const x = cfg.左边距;
    const y = this.cssHeight - cfg.下边距 - cfg.尺寸;
    const labelX = x + cfg.尺寸 + cfg.标签间距;
    this.ctx.save();
    this.ctx.font = cfg.字体;
    const textW = this.ctx.measureText(cfg.名称).width;
    this.ctx.restore();
    const w = cfg.尺寸 + cfg.标签间距 + textW + cfg.内边距;
    const h = cfg.尺寸;
  ...
  _排序区矩形() {
    const c = this.配置.排序区;
    return {
      x: this.cssWidth - c.右边距 - c.宽,
      y: this.cssHeight - c.下边距 - c.高,
      w: c.宽,
      h: c.高,
    };
  }
  ...
  _刷新芯片目标(force = false) {
    for (let i = 0; i < this.热区顺序.length; i++) {
      const name = this.热区顺序[i];
      const slot = this._槽位矩形(i);
      const s = this.芯片状态[name];
      s.目标x = slot.x;
      s.目标y = slot.y;
      if (force && this.拖拽芯片 !== name) {
        s.x = slot.x;
        s.y = slot.y;
      }
    }
  ...
  _更新拖拽排序(mouseY) {
    ...
    const from = this.热区顺序.indexOf(this.拖拽芯片);
    if (from === target) return;
    this.热区顺序.splice(from, 1);
    this.热区顺序.splice(target, 0, this.拖拽芯片);
    this._刷新芯片目标(false);
  }
  ...
  _更新芯片动画() {
    ...
    for (const name of names) {
      if (name === this.拖拽芯片) continue;
      const s = this.芯片状态[name];
      s.x += (s.目标x - s.x) * c.动画系数;
      s.y += (s.目标y - s.y) * c.动画系数;
      if (Math.abs(s.目标x - s.x) > c.动画阈值 || Math.abs(s.目标y - s.y) > c.动画阈值) need = true;
      else {
        s.x = s.目标x;
        s.y = s.目标y;
      }
  ...
  _按优先级解析命中(p) {
    const 命中角 = this._命中角热区(p);
    const 命中边 = this._命中边热区(p);
    const 命中本体 = this._命中本体热区(p);

    let 分组 = null;
    for (const name of this.热区顺序) {
      if (name === "角" && 命中角 >= 0) {
        分组 = "角";
        break;
      }
    ...
    this.悬停分组 = 分组;
    this.悬停角索引 = 分组 === "角" ? 命中角 : -1;
    this.悬停边索引 = 分组 === "边" ? 命中边 : -1;
  }

  鼠标按下(e) {
    this.更新边界矩形();
    const p = this.获取画布坐标(e);
    ...
    const 命中芯片 = this._命中芯片(p);
    if (命中芯片) {
      this.拖拽芯片 = 命中芯片;
      this.悬停芯片 = 命中芯片;
      const s = this.芯片状态[命中芯片];
      this.拖拽偏移.x = p.x - s.x;
      this.拖拽偏移.y = p.y - s.y;
      this.请求绘制();
      return;
  ...
  鼠标移动(e) {
    const p = this.获取画布坐标(e);
    const prev分组 = this.悬停分组;
    const prev角 = this.悬停角索引;
    const prev边 = this.悬停边索引;
    const prev复选框 = this.悬停复选框;
    const prev芯片 = this.悬停芯片;

    if (this.拖拽芯片) {
      const s = this.芯片状态[this.拖拽芯片];
      s.x = p.x - this.拖拽偏移.x;
      s.y = p.y - this.拖拽偏移.y;
      this._更新拖拽排序(p.y);
      this._按优先级解析命中(p);
    ...
    this._按优先级解析命中(p);
    this.悬停复选框 = this._命中复选框(p);
    this.悬停芯片 = this._命中芯片(p);
  ...
  _绘制热区遮罩() {
    ...
    ctx.save();
    ctx.setLineDash(热.虚线);

    const t = 热.边命中厚度;
    const half = t / 2;
    ctx.fillStyle = 色.热区边填充;
    ctx.strokeStyle = 色.热区边描边;
    const 边区 = [
      { x: r.x, y: r.y - half, w: r.w, h: t },
      { x: r.x + r.w - half, y: r.y, w: t, h: r.h },
      { x: r.x, y: r.y + r.h - half, w: r.w, h: t },
      { x: r.x - half, y: r.y, w: t, h: r.h },
    ];
    for (const e of 边区) {
      ctx.beginPath();
    ...
    ctx.fillStyle = 色.热区角填充;
    ctx.strokeStyle = 色.热区角描边;
    for (const c of 角点) {
      ctx.beginPath();
      ctx.arc(c.x, c.y, 热.角命中半径, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
    }
    ctx.restore();
  ...
  _绘制矩形与高亮() {
    ...
    ctx.fillStyle = this.悬停分组 === "本体" ? 色.本体高亮填充 : 色.本体填充;
    ...
    for (let i = 0; i < 4; i++) {
      const isHover = this.悬停分组 === "边" && this.悬停边索引 === i;
      ctx.beginPath();
      ctx.lineWidth = isHover ? 热.边高亮线宽 : 热.边线宽;
      ctx.strokeStyle = isHover ? 色.边高亮 : 色.边;
      ctx.moveTo(边线[i].x1, 边线[i].y1);
      ctx.lineTo(边线[i].x2, 边线[i].y2);
      ctx.stroke();
    }

    for (let i = 0; i < 4; i++) {
      const isHover = this.悬停分组 === "角" && this.悬停角索引 === i;
      ctx.beginPath();
      ctx.fillStyle = isHover ? 色.角高亮 : 色.角;
      ctx.arc(角点[i].x, 角点[i].y, isHover ? 热.角高亮半径 : 热.角可视半径, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ...
  _绘制复选框() {
    ...
    const c = this.配置.复选框;
    const x = c.左边距;
    const y = this.cssHeight - c.下边距 - c.尺寸;
    const hover = this.悬停复选框;

    ctx.save();
    ctx.fillStyle = this.显示热区 ? c.选中色 : hover ? c.背景悬停色 : c.背景色;
    ctx.strokeStyle = this.显示热区 ? c.选中色 : hover ? c.描边悬停色 : c.描边色;
    ctx.lineWidth = 1.5;
    roundRect(ctx, x, y, c.尺寸, c.尺寸, c.圆角, true, true);
    if (this.显示热区) {
      ctx.strokeStyle = c.勾线色;
      ctx.lineWidth = 2.2;
      ctx.beginPath();
      ctx.moveTo(x + 4, y + 9);
      ctx.lineTo(x + 8, y + 13);
    ...
    ctx.font = c.字体;
    ctx.textBaseline = "top";
    ctx.fillText(c.名称, x + c.尺寸 + c.标签间距, y + 2);
  }
  ...
  _绘制排序区() {
    ...
    ctx.lineWidth = 1;
    roundRect(ctx, p.x, p.y, p.w, p.h, c.圆角, true, true);

    ctx.fillStyle = c.标题色;
    ctx.font = c.标题字体;
    ctx.textBaseline = "top";
    ctx.textAlign = "left";
    ctx.fillText(c.标题, p.x + 30, p.y + 16);
    ...
    const 绘制芯片 = (name, isDragging) => {
      const s = this.芯片状态[name];
      const st = this.配置.分组样式[name];
      ctx.save();
      ctx.globalAlpha = isDragging ? c.拖拽透明度 : 1;
      ctx.fillStyle = st.填充;
      ctx.strokeStyle = st.描边;
      ctx.lineWidth = isDragging || this.悬停芯片 === name ? 2 : 1;
      roundRect(ctx, s.x, s.y, c.芯片宽, c.芯片高, c.芯片圆角, true, true);
      ctx.fillStyle = st.文字;
      ctx.font = c.芯片字体;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(name, s.x + c.芯片宽 / 2, s.y + c.芯片高 / 2);
      ctx.restore();
    };
    ...
    if (this.拖拽芯片) 绘制芯片(this.拖拽芯片, true);
      ...
      this.拖拽芯片 || this.悬停芯片
        ...
        : this.悬停复选框
          ...
          : this.悬停分组
            ? 光标.指向
            : 光标.默认;
    this.cvs.style.cursor = 光标样式;

    if (动画中) this.请求绘制();
  }
}

const 热区优先级实例 = new 热区优先级();
...
class 几何角和离心角 {
        ...
        absSin >= 0.85
          ...
          : absSin <= 0.55
            ? 是否对角
              ? 对角扩展系数
              : 1
            : 平滑(absSin, 0.55, 0.85, 是否对角 ? 对角扩展系数 : 1, 垂直压缩系数);
      const 左右外扩系数 =
        absCos >= 0.85
          ? 1
          : absCos <= 0.55
            ? 是否对角
              ? 对角扩展系数
              : 1
            : 平滑(absCos, 0.55, 0.85, 是否对角 ? 对角扩展系数 : 1, 1);
      const 几何角度 = 45;
  ...
  鼠标移动(事件) {
    ...
    if (上次悬停几何 !== this.悬停几何角 || 上次悬停参数 !== this.悬停离心角 || 上次悬停滑块 !== this.悬停滑块) {
      this.绘制();
    }
    this.画布.style.cursor = this.悬停几何角 || this.悬停离心角 || this.悬停滑块 != null ? 光标.指向 : 光标.默认;
  }
