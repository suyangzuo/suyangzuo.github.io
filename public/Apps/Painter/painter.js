class 随心绘 {
  ...
  获取多选整体旋转状态() {
    ...
    if (!边界) return { 整体旋转弧度: 0, 未旋转边界: null, 未旋转边界中心: null };
    ...
    // 获取未旋转的边界（优先使用保存的未旋转边界）
    let 未旋转边界 = 边界;
    if (this.全局标志.多选旋转中 && this.全局属性.多选旋转初始边界) {
      // 旋转中：使用保存的初始边界
      未旋转边界 = this.全局属性.多选旋转初始边界;
    } else if (已编组) {
      // 编组且不在旋转中：使用保存的编组未旋转边界
      const 组ID = this.全局属性.多选形状组[0].所属组ID;
      if (组ID && this.全局属性.编组旋转边界映射[组ID]) {
        未旋转边界 = this.全局属性.编组旋转边界映射[组ID];
      }
    } else {
      // 多选且不在旋转中：使用保存的多选未旋转边界
      const 多选标识 = this.全局属性.多选形状组
        .map((形状) => this.数据集.基础形状对象组.indexOf(形状))
        .sort()
        .join(",");
      if (this.全局属性.多选旋转边界映射[多选标识]) {
        未旋转边界 = this.全局属性.多选旋转边界映射[多选标识];
      }
    }

    const 未旋转边界中心 = {
      x: 未旋转边界.minX + 未旋转边界.width / 2,
      y: 未旋转边界.minY + 未旋转边界.height / 2,
    };

    return { 整体旋转弧度, 未旋转边界, 未旋转边界中心 };
  }
  ...
  鼠标位于多选整体交互框内() {
    ...
    const { 整体旋转弧度, 未旋转边界, 未旋转边界中心 } = this.获取多选整体旋转状态();
    if (!未旋转边界) return false;

    const { minX, minY, width, height } = 未旋转边界;
    const 外边距 = 25; // 与绘制时使用的外边距一致
    const 容差 = 10; // 额外的容差

    // 将鼠标坐标转换到未旋转的坐标系中
    let 局部鼠标X = this.全局属性.鼠标坐标.x;
    let 局部鼠标Y = this.全局属性.鼠标坐标.y;
    if (整体旋转弧度 !== 0 && 未旋转边界中心) {
      const dx = 局部鼠标X - 未旋转边界中心.x;
      const dy = 局部鼠标Y - 未旋转边界中心.y;
      const cos = Math.cos(-整体旋转弧度);
      const sin = Math.sin(-整体旋转弧度);
      局部鼠标X = 未旋转边界中心.x + dx * cos - dy * sin;
      局部鼠标Y = 未旋转边界中心.y + dx * sin + dy * cos;
    }

    // 检查鼠标是否在整体交互框的范围内（包括容差）
    return (
      局部鼠标X >= minX - 外边距 - 容差 &&
      局部鼠标X <= minX + width + 外边距 + 容差 &&
      局部鼠标Y >= minY - 外边距 - 容差 &&
      局部鼠标Y <= minY + height + 外边距 + 容差
    );
  }

  // 绘制多选整体交互框
  绘制多选整体交互框(边界, 不透明度 = 1.0, 强制已编组 = false) {
    if (!边界) return;
    ...
    // 如果整体交互框需要旋转，应用旋转变换
    // 使用未旋转边界的中心作为旋转中心（这样旋转后交互框才会正确显示）
    const 未旋转边界中心 = {
      x: 未旋转边界.minX + 未旋转边界.width / 2,
      y: 未旋转边界.minY + 未旋转边界.height / 2,
    };
    if (整体旋转弧度 !== 0) {
      this.ctx.translate(未旋转边界中心.x, 未旋转边界中心.y);
      this.ctx.rotate(整体旋转弧度);
      this.ctx.translate(-未旋转边界中心.x, -未旋转边界中心.y);
    }
  ...
  鼠标位于多选整体交互框边界() {
    ...
    // 检查是否在句柄上（四个角）
    const 句柄半径 = 8;
    const 左上距离 = Math.sqrt(Math.pow(局部鼠标X - 左边界, 2) + Math.pow(局部鼠标Y - 上边界, 2));
    const 右上距离 = Math.sqrt(Math.pow(局部鼠标X - 右边界, 2) + Math.pow(局部鼠标Y - 上边界, 2));
    const 右下距离 = Math.sqrt(Math.pow(局部鼠标X - 右边界, 2) + Math.pow(局部鼠标Y - 下边界, 2));
    const 左下距离 = Math.sqrt(Math.pow(局部鼠标X - 左边界, 2) + Math.pow(局部鼠标Y - 下边界, 2));
    ...
    // 检查是否在边界上（上下左右边）
    const 在水平范围内 = 局部鼠标X >= 左边界 - 容差 && 局部鼠标X <= 右边界 + 容差;
    const 在垂直范围内 = 局部鼠标Y >= 上边界 - 容差 && 局部鼠标Y <= 下边界 + 容差;

    const 边界位置 = {
      上: !位于句柄 && 在水平范围内 && 局部鼠标Y >= 上边界 - 容差 && 局部鼠标Y <= 上边界 + 容差,
      下: !位于句柄 && 在水平范围内 && 局部鼠标Y >= 下边界 - 容差 && 局部鼠标Y <= 下边界 + 容差,
      左: !位于句柄 && 在垂直范围内 && 局部鼠标X >= 左边界 - 容差 && 局部鼠标X <= 左边界 + 容差,
      右: !位于句柄 && 在垂直范围内 && 局部鼠标X >= 右边界 - 容差 && 局部鼠标X <= 右边界 + 容差,
    };
  ...
  执行多选缩放() {
    ...
    if (缩放模式 === "horizontal") {
      ...
      const 当前垂直距离 = 鼠标坐标.y - 缩放锚点.y;
      垂直缩放比例 = Math.abs(初始垂直距离) > 阈值 ? 当前垂直距离 / 初始垂直距离 : 1;
    } else if (缩放模式 === "proportional" || this.键盘状态.Shift) {
      // 等比缩放
      const 初始距离 = Math.sqrt(Math.pow(点击坐标.x - 缩放锚点.x, 2) + Math.pow(点击坐标.y - 缩放锚点.y, 2));
      const 当前距离 = Math.sqrt(Math.pow(鼠标坐标.x - 缩放锚点.x, 2) + Math.pow(鼠标坐标.y - 缩放锚点.y, 2));
      const 缩放比例 = 初始距离 > 阈值 ? 当前距离 / 初始距离 : 1;
  ...
  应用多选形状缩放(形状, 初始位置, 水平缩放比例, 垂直缩放比例, 缩放锚点, 初始边界) {
    ...
    // 计算形状在初始边界中的相对位置（相对于初始边界的中心）
    const 初始中心X = 初始边界.centerX;
    const 初始中心Y = 初始边界.centerY;

    // 获取形状的初始中心点
    let 形状初始中心 = null;
    if (形状.形状 === "圆" || 形状.形状 === "多边形" || 形状.形状 === "多角星") {
      形状初始中心 = { x: 初始位置.按下时坐标.x, y: 初始位置.按下时坐标.y };
    } else if (形状.形状 === "文本") {
      // 文本形状：计算初始几何中心
      if (初始位置.按下时顶点坐标组 && 初始位置.按下时顶点坐标组.length >= 4) {
        const 初始顶点 = 初始位置.按下时顶点坐标组;
        形状初始中心 = {
          x: (初始顶点[0].x + 初始顶点[1].x + 初始顶点[2].x + 初始顶点[3].x) / 4,
          y: (初始顶点[0].y + 初始顶点[1].y + 初始顶点[2].y + 初始顶点[3].y) / 4,
        };
      } else {
        形状初始中心 = { x: 初始位置.按下时坐标.x, y: 初始位置.按下时坐标.y };
      }
    } else {
      // 其他形状：使用极值坐标的中心（需要重新计算）
      if (初始位置.按下时顶点坐标组 && 初始位置.按下时顶点坐标组.length > 0) {
        let minX = Infinity,
          minY = Infinity,
          maxX = -Infinity,
          maxY = -Infinity;
        初始位置.按下时顶点坐标组.forEach((顶点) => {
          minX = Math.min(minX, 顶点.x);
          minY = Math.min(minY, 顶点.y);
          maxX = Math.max(maxX, 顶点.x);
          maxY = Math.max(maxY, 顶点.y);
        });
        形状初始中心 = { x: (minX + maxX) / 2, y: (minY + maxY) / 2 };
      } else {
        形状初始中心 = { x: 初始位置.按下时坐标.x, y: 初始位置.按下时坐标.y };
      }
    }
    ...
    // 计算形状相对于初始边界中心的偏移
    const 形状相对偏移X = 形状初始中心.x - 初始中心X;
    const 形状相对偏移Y = 形状初始中心.y - 初始中心Y;

    // 计算缩放后形状的新中心点
    // 首先计算初始边界中心到缩放锚点的向量
    const 初始中心到锚点X = 缩放锚点.x - 初始中心X;
    const 初始中心到锚点Y = 缩放锚点.y - 初始中心Y;

    // 缩放后，边界中心的新位置
    const 新边界中心X = 缩放锚点.x - 初始中心到锚点X * 水平缩放比例;
    const 新边界中心Y = 缩放锚点.y - 初始中心到锚点Y * 垂直缩放比例;

    // 形状的新中心点
    const 新形状中心X = 新边界中心X + 形状相对偏移X * 水平缩放比例;
    const 新形状中心Y = 新边界中心Y + 形状相对偏移Y * 垂直缩放比例;
    ...
    if (形状.形状 === "文本") {
      ...
      let 原始缩放比例;
      if (句柄越过另一端) {
        原始缩放比例 = 12 / 初始位置.按下时字号;
      } else {
        原始缩放比例 = (Math.abs(水平缩放比例) + Math.abs(垂直缩放比例)) / 2;
      }
      const 目标字号 = Math.round(初始位置.按下时字号 * 原始缩放比例);
      形状.字号 = Math.max(12, Math.min(100, 目标字号));
      const 实际缩放比例 = 形状.字号 / 初始位置.按下时字号;
      ...
      if (初始位置.按下时顶点坐标组 && 初始位置.按下时顶点坐标组.length >= 4) {
        ...
        形状.顶点坐标组 = 初始位置.按下时顶点坐标组.map((顶点) => ({
          x: 文本初始中心.x + (顶点.x - 文本初始中心.x) * 实际缩放比例,
          y: 文本初始中心.y + (顶点.y - 文本初始中心.y) * 实际缩放比例,
        }));

        // 将缩放后的文本中心移动到新位置
        const 当前文本中心 = {
          x: (形状.顶点坐标组[0].x + 形状.顶点坐标组[1].x + 形状.顶点坐标组[2].x + 形状.顶点坐标组[3].x) / 4,
          y: (形状.顶点坐标组[0].y + 形状.顶点坐标组[1].y + 形状.顶点坐标组[2].y + 形状.顶点坐标组[3].y) / 4,
        };
        const 偏移X = 新形状中心X - 当前文本中心.x;
        const 偏移Y = 新形状中心Y - 当前文本中心.y;
        形状.顶点坐标组 = 形状.顶点坐标组.map((顶点) => ({
          x: 顶点.x + 偏移X,
          y: 顶点.y + 偏移Y,
        }));
      ...
      // 圆形：缩放半径并更新坐标
      if (初始位置.按下时尺寸) {
        形状.尺寸.水平半径 = Math.abs(初始位置.按下时尺寸.水平半径 * 水平缩放比例);
        形状.尺寸.垂直半径 = Math.abs(初始位置.按下时尺寸.垂直半径 * 垂直缩放比例);
      }
      形状.坐标.x = 新形状中心X;
      形状.坐标.y = 新形状中心Y;
      // 更新顶点坐标组（如果需要）
      ...
      // 多角星：缩放外半径和内半径并更新坐标
      if (初始位置.按下时尺寸 && 初始位置.按下时尺寸.外半径 && 初始位置.按下时尺寸.内半径) {
        形状.尺寸.外半径.水平 = Math.abs(初始位置.按下时尺寸.外半径.水平 * 水平缩放比例);
        形状.尺寸.外半径.垂直 = Math.abs(初始位置.按下时尺寸.外半径.垂直 * 垂直缩放比例);
        形状.尺寸.内半径.水平 = Math.abs(初始位置.按下时尺寸.内半径.水平 * 水平缩放比例);
        形状.尺寸.内半径.垂直 = Math.abs(初始位置.按下时尺寸.内半径.垂直 * 垂直缩放比例);
      }
      形状.坐标.x = 新形状中心X;
      形状.坐标.y = 新形状中心Y;
      // 重新计算外顶点和内顶点坐标组
      ...
      // 其他形状（矩形、图像、直线、自由、箭头）：缩放顶点坐标组
      if (初始位置.按下时顶点坐标组 && 初始位置.按下时顶点坐标组.length > 0) {
        // 计算形状的初始中心（相对于边界中心）
        let 顶点初始中心 = null;
        if (初始位置.按下时顶点坐标组.length >= 4) {
          顶点初始中心 = {
            x:
              (初始位置.按下时顶点坐标组[0].x +
                初始位置.按下时顶点坐标组[1].x +
        ...
        } else if (初始位置.按下时顶点坐标组.length === 2) {
          // 直线
          顶点初始中心 = {
            x: (初始位置.按下时顶点坐标组[0].x + 初始位置.按下时顶点坐标组[1].x) / 2,
            y: (初始位置.按下时顶点坐标组[0].y + 初始位置.按下时顶点坐标组[1].y) / 2,
          };
        }
        ...
        if (顶点初始中心) {
          // 缩放顶点（相对于顶点初始中心）
          形状.顶点坐标组 = 初始位置.按下时顶点坐标组.map((顶点) => ({
            x: 顶点初始中心.x + (顶点.x - 顶点初始中心.x) * 水平缩放比例,
            y: 顶点初始中心.y + (顶点.y - 顶点初始中心.y) * 垂直缩放比例,
          }));

          // 将缩放后的形状中心移动到新位置
          const 当前形状中心 = {
            x: 形状.顶点坐标组.reduce((sum, v) => sum + v.x, 0) / 形状.顶点坐标组.length,
            y: 形状.顶点坐标组.reduce((sum, v) => sum + v.y, 0) / 形状.顶点坐标组.length,
          };
          const 偏移X = 新形状中心X - 当前形状中心.x;
          const 偏移Y = 新形状中心Y - 当前形状中心.y;
          形状.顶点坐标组 = 形状.顶点坐标组.map((顶点) => ({
            x: 顶点.x + 偏移X,
            y: 顶点.y + 偏移Y,
          }));
  ...
  旋转多选形状90度(顺时针) {
    ...
    if (是否为编组) {
      ...
      const 多选标识 = this.全局属性.多选形状组.map(形状 => this.数据集.基础形状对象组.indexOf(形状)).sort().join(',');
      if (this.全局属性.多选旋转边界映射[多选标识]) {
        初始边界 = this.全局属性.多选旋转边界映射[多选标识];
      }
    }
      ...
      if (形状.形状 === "矩形" || 形状.形状 === "图像" || 形状.形状 === "箭头") {
        ...
        if (初始位置.按下时顶点坐标组 && 初始位置.按下时顶点坐标组.length > 0) {
          ...
          if (形状.形状 === "矩形" || 形状.形状 === "图像") {
            ...
            let 未旋转尺寸 = { 宽: 0, 高: 0 };
            if (初始旋转弧度 !== 0) {
              const 未旋转顶点组 = 初始顶点组.map((顶点) =>
                this.旋转坐标(顶点, 初始中心, -初始旋转弧度)
              );
              const 未旋转最左 = Math.min(...未旋转顶点组.map((v) => v.x));
              const 未旋转最右 = Math.max(...未旋转顶点组.map((v) => v.x));
              const 未旋转最上 = Math.min(...未旋转顶点组.map((v) => v.y));
    ...
    if (是否为编组) {
      ...
      整体旋转状态 = {
        ...
        多选标识: 多选标识,
        之前的旋转弧度: 操作前的旋转弧度,
        之前的边界: this.全局属性.多选旋转边界映射[多选标识] ? structuredClone(this.全局属性.多选旋转边界映射[多选标识]) : null,
      };
    }
    ...
    if (
      ...
      if (组内形状 && 组内形状.length >= 2) {
        ...
        if (边界 && 边界.width > 0 && 边界.height > 0) {
          // 绘制整体预览交互框（不透明度50%，强制已编组为true）
          this.绘制多选整体交互框(边界, 0.35, true);
        }
  ...
  绘制交互框(形状对象, 透明度 = 1, 显示句柄 = true) {
    ...
    const 虚线长度 = 形状对象.已多选 && 形状对象.已悬停 && this.全局标志.Shift已按下 ? 5 : 10;
    this.ctx.lineWidth = 1;
